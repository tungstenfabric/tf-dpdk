...
typedef struct n3k_mgmt_flow_entry (*create_flow_entry_t)(
	uint16_t src_port, uint16_t dst_port, enum key_address_type kat);
...
create_local_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
...
static struct n3k_mgmt_flow_entry
create_local_mirror_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct n3k_mgmt_flow_tbl_key *key = &flow.key.key_raw;
...
static struct n3k_mgmt_flow_entry
create_encap_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
...
static struct n3k_mgmt_flow_entry
create_encap_vxlan_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct rte_ether_addr *smac = &flow.action.tunnel.smac_raw;
...
static struct n3k_mgmt_flow_entry
create_drop_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct n3k_mgmt_flow_tbl_key *key = &flow.key.key_raw;
...
static struct n3k_mgmt_flow_entry
create_decap_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
...
static struct n3k_mgmt_flow_entry
create_decap_vxlan_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct n3k_mgmt_vplkp_tbl_res *vplkp = &flow.key.vplkp_raw;
...
static struct n3k_mgmt_flow_entry
create_encap_l3_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct rte_ether_addr *smac = &flow.action.tunnel.smac_raw;
...
static struct n3k_mgmt_flow_entry
create_decap_l3_flow_entry(uint16_t src_port, uint16_t dst_port, enum key_address_type kat)
{
	struct n3k_mgmt_flow_entry flow;
	memset(&flow, 0, sizeof(flow));
	struct n3k_mgmt_vplkp_tbl_res *vplkp = &flow.key.vplkp_raw;
...
static uint32_t max_flows_count;

static void
schedule_flow_add(struct n3k_mgmt_hw *hw, enum n3k_flow_entry_type flow_type,
	uint32_t flow_cnt, enum key_address_type kat)
{
	int ret;
	static uint16_t src_port = 0, dst_port = 0;

	if (flow_cnt > max_flows_count) {
		printf("Too many flows. Trimming to %" PRIu32 "\n", max_flows_count);
		fflush(stdout);
		flow_cnt = max_flows_count;
	}
	...
	while (flow_cnt--) {
		...
		struct n3k_mgmt_flow_entry_description tmp_desc;
		...
		cmd->flow_create.flow = create_entry(src_port, dst_port, kat);

		ret = n3k_mgmt_flr_flow_schedule_add(hw, &cmd->flow_create, &tmp_desc);
		if (ret < 0) {
			rte_exit(EXIT_FAILURE,
				"n3k_mgmt_flr_flow_schedule_add failed ret=%d(%s)\n",
				ret, strerror(-ret));
		}
		flow_descs[tmp_desc.flow_handle->flow_id] = tmp_desc;
...
schedule_flow_query(struct n3k_mgmt_hw *hw, uint32_t flow_id_beg,
	uint32_t flow_cnt)
{
	uint64_t flow_id_end = (uint64_t)flow_id_beg + (uint64_t)flow_cnt;
	struct n3k_mgmt_flr_command **cmds;
	uint64_t i;
	int ret;

	if ((flow_id_beg >= max_flows_count) || (flow_cnt > max_flows_count) || (flow_id_end > max_flows_count)) {
		printf("values out of range( %" PRIu32 " + %" PRIu32 " >= %" PRIu32 " )\n",
			flow_id_beg, flow_cnt, max_flows_count);
		fflush(stdout);
		return;
	}
	...
	for (i = flow_id_beg; i < flow_id_end; ++i) {
		...
		if (ret < 0) {
			printf("Failed to gen flow statistics for flow_id=%" PRIu64 "\n", i);
		} else {
			printf("flow_id=%" PRIu64 " ", i);
...
read_cmdline(struct n3k_mgmt_hw *hw)
{
	while (1) {
		...
		if (strncmp(command_type_str, "add", 3) == 0) {
			...
			schedule_flow_add(hw, flow_type, flow_cnt,
					command_type_str[3] == '6' ? IPv6 : IPv4);
		} else if (strncmp(command_type_str, "query", 5) == 0) {
		...
		} else if (strncmp(command_type_str, "del", 3) == 0) {
			uint32_t flow_id_beg, flow_cnt;
			ret = scanf("%" SCNu32 "%" SCNu32, &flow_id_beg, &flow_cnt);
			if (ret == EOF)
				break;
...
main(int argc, char *argv[])
{
	...
	enum n3k_mgmt_hw_insert_type insert_type = hw->insert_type;
	...
	max_flows_count = n3k_mgmt_flow_tbl_get_max_flow_id(hw);
	flow_descs = rte_zmalloc("n3k-flr-test-app", sizeof(*flow_descs) * max_flows_count, 0);
	read_cmdline(hw);
	rte_free(flow_descs);
