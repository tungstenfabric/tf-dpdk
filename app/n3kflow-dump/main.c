...
{
	memset(msg, 0, sizeof(*msg));
	strncpy(msg->name, N3K_MGMT_MP_REQUEST_VF0_STATS, RTE_MP_MAX_NAME_LEN);

	msg->len_param = sizeof(n3k_dump_options.mgmt_device_name);
	memcpy(msg->param, n3k_dump_options.mgmt_device_name, sizeof(n3k_dump_options.mgmt_device_name));
...
n3k_dump_handle_request_vf0_stats(void)
{
	...
	struct rte_mp_msg *rte_response;
	struct n3k_mgmt_mp_vf0_stats_response *vf0_stats_res;

	prepare_n3k_vf0_stats_request(&msg, &response, &timespec,
		N3K_DUMP_DEFAULT_TIMEOUT);
	...
	rte_response = &response.msgs[0];
	vf0_stats_res = (struct n3k_mgmt_mp_vf0_stats_response *)(&rte_response->param[0]);

	if (n3k_dump_options.json) {
		printf(" ");
		n3k_dump_mp_json_vf0_stats_dump(vf0_stats_res);
		printf("\n");
	} else {
		rte_pmd_n3k_mp_vf0_stats_dump(vf0_stats_res);
	}
