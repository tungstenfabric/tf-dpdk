...
static const char *n3k_dump_usage_msg[] = {
	"Usage: n3k-dump <EAL parameters> -- "
	"<--" DEVICE_NAME_STR " name>",
	"<--" TABLE_TYPE_STR " type> [-t] [-r] [-j]",
	"or",
	"<--" DUMP_REGISTER_STR ">  <--" BASE_STR
	" base_addr>  <--" OFFSET_STR " offset_addr> [--" DEBUG_REGS_STR "]",
	"",
	...
	while ((ret = getopt_long(argc, argv, n3k_dump_short_options,
		...
		case 0:
			if (!strcmp(n3k_dump_long_options[opt_idx].name, DEVICE_NAME_STR)) {
				...
				if (dev_name_len > N3K_MGMT_MP_DEVICE_MAX_LEN) {
					N3K_DUMP_LOG(ERR, "invalid device name length %zu",
						dev_name_len);
					return -EINVAL;
				}
				strncpy(opts->mgmt_device_name, optarg, N3K_MGMT_MP_DEVICE_MAX_LEN);
