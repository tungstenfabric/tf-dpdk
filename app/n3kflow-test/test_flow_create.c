...
static void
assert_inner_key_matches(
	struct n3k_mgmt_flow_tbl_key *key, uint8_t proto, bool use_l2)
{
	if (use_l2) {
	...
	assert_int_equal(key->l3.type, N3K_MGMT_FLOW_TBL_L3_TYPE_IPV4);
...
static void
assert_outer_key_matches(struct n3k_mgmt_flow_key *key, bool use_l2)
{
...
static void
assert_encap_action_matches(struct n3k_mgmt_flow_action *action, bool is_l3)
{
	struct n3k_mgmt_flow_tbl_action *action_raw = &action->action_raw;

	assert_int_equal(
		action_raw->encap_action.type, N3K_MGMT_TUNNEL_TYPE_MPLSOUDP);
...
test_local_udp_l2_forward(void **state __rte_unused)
{
	...
	assert_inner_key_matches(key, proto, true);
...
test_egress_mplsoudp_udp_l2_forward(void **state __rte_unused)
{
	...
	assert_encap_action_matches(&flow.action, false);
}
...
test_ingress_mplsoudp_udp_l2_forward(void **state __rte_unused)
{
	...
	assert_inner_key_matches(key, proto, true);
	assert_outer_key_matches(&flow.key, true);
...
test_egress_mplsoudp_udp_l3_forward(void **state __rte_unused)
{
	...
	assert_encap_action_matches(&flow.action, true);
}
...
test_ingress_mplsoudp_udp_l3_forward(void **state __rte_unused)
{
	...
	assert_int_equal(key->port.port_id, SRC_PORT_ID);
	assert_true(key->use_tunnel_key);
	assert_inner_key_matches(key, proto, false);
	assert_outer_key_matches(&flow.key, false);
