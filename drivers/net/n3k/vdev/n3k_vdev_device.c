	...
	const struct rte_eth_dev *pf_dev, const struct rte_eth_dev *vf_dev)
{
	...
	if (pf_dev->data->nb_rx_queues != vf_dev->data->nb_rx_queues)
		return false;

	if (pf_dev->data->nb_tx_queues != vf_dev->data->nb_tx_queues)
		return false;
...
n3k_vdev_dev_configure(struct rte_eth_dev *dev)
{
	...
	N3K_VDEV_LOG(DEBUG, "Configuring %s", dev->data->name);
...
n3k_vdev_dev_start(struct rte_eth_dev *dev)
{
	...
	N3K_VDEV_LOG(DEBUG, "Starting %s", dev->data->name);

	if (!dev_internal->configured) {
		N3K_VDEV_LOG(ERR, "Device %s cannot be started before it is configured",
			dev->data->name);
		return -ENOTSUP;
	} else if (!n3k_vdev_check_if_hardware_config_matches(pf_dev, dev)) {
		N3K_VDEV_LOG(ERR,
	...
	if (!pf_dev->data->dev_started) {
		N3K_VDEV_LOG(DEBUG, "PF was stopped. Starting...");

		ret = rte_eth_dev_start(pf_dev->data->port_id);
		if (ret) {
			N3K_VDEV_LOG(ERR, "Failed to start PF %s for VDEV %s",
...
n3k_vdev_dev_stop(struct rte_eth_dev *dev)
{
	...
	N3K_VDEV_LOG(DEBUG, "Stopping %s", dev->data->name);

	dev_shared->started_vdevs_count--;

	if (dev_shared->started_vdevs_count == 0) {
		N3K_VDEV_LOG(DEBUG, "All VDEVs stopped. Stopping PF and dispatcher.");
...
n3k_vdev_mac_addr_set(struct rte_eth_dev *dev, struct rte_ether_addr *mac_addr)
{
	if (!rte_is_valid_assigned_ether_addr(mac_addr)) {
		N3K_VDEV_LOG(ERR, "Tried to set invalid MAC address");
		return -EINVAL;
	}
...
n3k_vdev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats)
{
	...
	stats->ibytes = n3k_stats.ibytes;
	stats->ipackets = n3k_stats.ipackets;
	stats->obytes = n3k_stats.obytes;
	stats->opackets = n3k_stats.opackets;
	...
	void *arg)
{
	if (filter_type == RTE_ETH_FILTER_GENERIC) {
		...
		*(const void **)arg = &n3k_vdev_flow_ops;
		...
		N3K_VDEV_LOG(WARNING, "Filter type (%d) not supported",
			filter_type);
		return -ENOTSUP;
	}
}
...
n3k_vdev_dev_init(struct rte_eth_dev *eth_dev, void *init_params)
{
	...
	if (eth_dev->data->mac_addrs == NULL) {
		N3K_VDEV_LOG(ERR, "Failed to allocate MAC address");
		return -ENOMEM;
	}
