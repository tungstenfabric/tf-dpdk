	...
	void *extra_args)
{
	...
	if (!dev) {
		N3K_VDEV_LOG(ERR, "Cannot find port by name %s", value);
		return -ENODEV;
	}
	...
	struct rte_kvargs *kvlist, const char *dev_name, struct n3k_vdev_args *args)
{
	...
	if (rte_kvargs_count(kvlist, N3K_VDEV_PF_KVARG) != 1) {
		N3K_VDEV_LOG(ERR, "PF must be specified only once for %s", dev_name);
		return -EINVAL;
	}
	...
	if (ret) {
		N3K_VDEV_LOG(ERR, "PF device parsing for %s failed", dev_name);
		return ret;
	}
...
n3k_vdev_parse_vf_id(const char *vf_str, void *extra_args)
{
	...
	if (vf_nb > N3K_VF_PORTS_MAX_COUNT || vf_nb < N3K_VF_PORT_MIN_ID){
		N3K_VDEV_LOG(ERR, "Vf id %s is not valid", vf_str);
		return -EINVAL;
	}
	...
	if (args->vfs_count > N3K_VF_PORTS_MAX_COUNT){
		N3K_VDEV_LOG(ERR, "Number of vfs higher than %d", N3K_VF_PORTS_MAX_COUNT);
		return -ERANGE;
	}
...
static int
n3k_vdev_parse_kvargs_list(const char *str, void *args,
	int (*callback)(const char*, void*))
{
	...
	if (strlen(str) == 0) {
		N3K_VDEV_LOG(ERR, "Argument is not provided");
		ret = -EINVAL;
		return ret;
	}

	if (*str != '[' || str[strlen(str) - 1] != ']') {
		N3K_VDEV_LOG(ERR, "Argument %s is not a list", str);
		ret = -EINVAL;
		return ret;
	}
	...
	if (dup_str == NULL) {
		N3K_VDEV_LOG(ERR, "Not enough memory");
		ret = -ENOMEM;
		return ret;
	}
	ptr = strtok(dup_str, N3K_VDEV_LIST_DELIM);

	while (ptr != NULL) {
		ret = callback(ptr, args);
		if (ret)
			goto free_dup_str;
		ptr = strtok(NULL, N3K_VDEV_LIST_DELIM);
	}
	...
	void *extra_args)
{
	...
	ret = n3k_vdev_parse_kvargs_list(value, args, n3k_vdev_parse_vf_id);
	if (ret) {
		N3K_VDEV_LOG(ERR, "Parsing of vfs argument failed");
		return ret;
	}
	...
	struct rte_kvargs *kvlist, const char *dev_name, struct n3k_vdev_args *args)
{
	...
	if (rte_kvargs_count(kvlist, N3K_VDEV_VFS_KVARG) != 1) {
		N3K_VDEV_LOG(ERR, "VFs list must be specified only once for %s", dev_name);
		return -EINVAL;
	}
	...
	if (ret) {
		N3K_VDEV_LOG(ERR, "VF devices parsing for %s failed", dev_name);
		return ret;
	}
	...
	struct rte_kvargs *kvlist, const char *dev_name, struct n3k_vdev_args *args)
{
	...
	if (rte_kvargs_count(kvlist, N3K_VDEV_MGMT_KVARG) != 1) {
		N3K_VDEV_LOG(ERR, "MGMT must be specified only once for %s", dev_name);
		return -EINVAL;
	}
	...
	if (ret) {
		N3K_VDEV_LOG(ERR, "MGMT device parsing for %s failed", dev_name);
		return ret;
	}
	...
	struct rte_kvargs *kvlist, const char *dev_name, struct n3k_vdev_args *args)
{
	...
	if (ret) {
		N3K_VDEV_LOG(ERR, "LAG switch parsing for %s failed", dev_name);
		return ret;
	}
...
n3k_vdev_args_parse(struct rte_vdev_device *dev, struct n3k_vdev_args *args)
{
	...
	if (!kvlist) {
		N3K_VDEV_LOG(ERR, "Failed to parse arguments for %s", name);
		return -EINVAL;
	}
