	...
	uint16_t queue_id, uint16_t count, int socket_id)
{
	...
	N3K_VDEV_LOG(DEFAULT, DEBUG, "New ring created: %s", ring_name);

	return rte_ring_create(
		ring_name, rte_align32pow2(count),
	...
	const struct rte_eth_rxconf *conf, struct rte_mempool *mb_pool)
{
	...
	N3K_VDEV_LOG(DEFAULT, DEBUG, "Setting up RX queue #%d for %s [Desc: %d; Socket: %d]",
	...
	if (!dev_internal->dedicated_queue_enabled || queue_id !=
			dev_internal->dedicated_queue_id) {
		if (dev_shared->configured_rx_queues_count[queue_id] == 0) {
			pf_mempool = &dev_shared->pf_rx_mempools[queue_id];

			/* TODO: Change after fixing virtio queues logic. Due to incorrect logic of queue management in virtio PMD this currently will crash. */
			if (!*pf_mempool)
			...
			if (ret) {
				N3K_VDEV_LOG(DEFAULT, ERR, "Failed to set up RX queue #%d for PF %s of %s",
					queue_id, pf_dev->data->name, dev->data->name);
			...
			ret = n3k_vdev_check_pf_rx_config(
				dev_shared, queue_id, nb_desc, socket_id, conf);
			if (ret) {
				N3K_VDEV_LOG(DEFAULT, ERR, "Configuration of RX queue #%d for %s does not "
	...
	if (dev_internal->dedicated_queue_enabled && queue_id == dev_internal->dedicated_queue_id)
		dev_shared->configured_dedicated_rx_queue_count++;
	else
...
error:
	/* TODO: Uncomment after fixing virtio queues logic. Due to incorrect logic of queue management in virtio PMD this currently will crash. */
	// if (pf_mempool)
	// 	n3k_vdev_pf_mempool_free(pf_mempool);
...
n3k_vdev_rx_queue_release(void *rx_queue)
{
	...
	N3K_VDEV_LOG(DEFAULT, DEBUG, "Releasing RX queue #%d of %s",
		queue->queue_id, queue->dev->data->name);

	dev_internal = queue->dev->data->dev_private;
	dev_shared = dev_internal->shared;

	if (dev_internal->dedicated_queue_enabled && queue->queue_id == dev_internal->dedicated_queue_id)
		dev_shared->configured_rx_queues_count[queue->queue_id]--;
	else
		dev_shared->configured_dedicated_rx_queue_count--;

	n3k_vdev_free_all_mbufs_from_ring(queue->ring_free);
	n3k_vdev_free_all_mbufs_from_ring(queue->ring_used);

	/* TODO: Uncomment after fixing virtio queues logic. Due to incorrect logic of queue management in virtio PMD this currently will crash. */
	// TODO: check if dedicated queue
	...
	const struct rte_eth_txconf *conf)
{
	...
	N3K_VDEV_LOG(DEFAULT, DEBUG, "Setting up TX queue #%d for %s [Desc: %d; Socket: %d]",
		queue_id, dev->data->name, nb_desc, socket_id);
	...
	if (dev_internal->dedicated_queue_enabled && queue_id ==
			dev_internal->dedicated_queue_id) {
		if (dev_shared->configured_dedicated_tx_queue_count == 0) {
			pf_mempool = &dev_shared->pf_dedicated_tx_mempool;
			if (!*pf_mempool)
				*pf_mempool = n3k_vdev_pf_mempool_alloc(
					pf_dev->device->name, "dedic_tx", queue_id, nb_desc, socket_id);

			if (!*pf_mempool) {
				N3K_VDEV_LOG(DEFAULT, ERR,
					"Failed to allocate mempool for dedicated TX queue for PF %s of %s",
					pf_dev->data->name, dev->data->name);
				ret = -ENOMEM;
				goto error;
			}
		}
		queue->mempool = dev_shared->pf_dedicated_tx_mempool;
	} else {
		if (dev_shared->configured_tx_queues_count[queue_id] == 0) {
			...
			if (ret) {
				N3K_VDEV_LOG(DEFAULT, ERR, "Failed to set up TX queue #%d for PF %s of %s",
					queue_id, pf_dev->data->name, dev->data->name);
				n3k_vdev_pf_mempool_free(pf_mempool);
				goto error;
			}
	...
	if (dev_internal->dedicated_queue_enabled && queue_id == dev_internal->dedicated_queue_id)
		dev_shared->configured_dedicated_tx_queue_count++;
	else
...
n3k_vdev_tx_queue_release(void *tx_queue)
{
	...
	N3K_VDEV_LOG(DEFAULT, DEBUG, "Releasing TX queue #%d of %s",
		queue->queue_id, queue->dev->data->name);
