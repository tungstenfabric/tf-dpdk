...
n3k_vdev_probe(struct rte_vdev_device *dev)
{
	...
	N3K_VDEV_LOG(INFO, "Initializing pmd_n3k for %s", name);
	...
	ret = n3k_mgmt_lag_enable(args.mgmt_dev, args.is_lag_enabled);
	if (ret) {
		N3K_VDEV_LOG(ERR, "%s LAG mode for %s failed",
	...
	for (i = 0; i < N3K_PHY_REPRESENTORS_COUNT; ++i) {
		...
		if (ret) {
			...
			N3K_VDEV_LOG(ERR, "Initializing phy%d for %s failed", i, name);
			return ret;
		}
	}
	...
	for (i = 0; i < args.vfs_count; ++i) {
		...
		if (ret) {
			/* TODO: Free all created */
			N3K_VDEV_LOG(ERR, "Initializing vf%d for %s failed",
				args.vfs[i], name);
			return ret;
		}
	}
	...
	if (!vdev_shared->dispatcher) {
		/* TODO: Free all */
		N3K_VDEV_LOG(ERR, "Initializing dispatcher for %s failed", name);
		return -ENOMEM;
	}
