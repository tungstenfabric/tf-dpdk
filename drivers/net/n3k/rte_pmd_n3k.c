...
rte_pmd_n3k_get_pci_addr(const struct rte_eth_dev *repr_dev, struct rte_pci_addr *repr_addr)
{
	...
	if (rte_pci_addr_parse(pf_name, &pf_addr)) {
		N3K_VDEV_LOG(DEFAULT, ERR, "Parsing vf0 VF's PCI address failed: %s", pf_name);
		return -EINVAL;
	}
...
int
rte_pmd_n3k_get_vdpa_did_by_repr_name(const char *name)
{
...
int
rte_pmd_n3k_enable_dedicated_queue_on_repr(struct rte_eth_dev *dev)
{
	...
	rte_eth_dev_info_get(dev->data->port_id, &dev_info);
	dev_internal->dedicated_queue_enabled = true;
	RTE_ASSERT(dev_info.max_rx_queues == dev_info.max_tx_queues);
	dev_internal->dedicated_queue_id = dev_info.max_rx_queues;

	return 0;
}

int
rte_pmd_n3k_disable_dedicated_queue_on_repr(struct rte_eth_dev *dev)
{
	struct n3k_vdev_dev_internal *dev_internal;
