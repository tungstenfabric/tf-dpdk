...
#define N3K_MGMT_MP_REQUEST_VF0_STATS "n3k_mgmt_mp_request_vf0_stats"
...
struct n3k_mgmt_mp_entries_request {
	enum n3k_mgmt_mp_request_table table;
};

struct n3k_mgmt_mp_vf0_stats_request {};

struct n3k_mgmt_mp_request {
	char device_name[RTE_ETH_NAME_MAX_LEN];
	// request type is specified in mp msg name
	union {
		struct n3k_mgmt_mp_entries_request entries_req; // same used for total entries
		struct n3k_mgmt_mp_register_request register_req;
		struct n3k_mgmt_mp_vf0_stats_request vf0_stats_req;
	};
};
...
struct n3k_mgmt_mp_register_response {
	uint32_t reg_value;
};

struct n3k_mgmt_mp_vf0_stats_response {
	struct {
		uint64_t rx_pkts;
		uint64_t tx_pkts;
	} slow_path, flr;
...
struct n3k_mgmt_mp_response {
	union {
		struct n3k_mgmt_mp_register_response register_resp;
		struct n3k_mgmt_mp_total_entries_response total_entries_resp;
...
void rte_pmd_n3k_mp_port_dump(const struct n3k_mgmt_mp_port_response *port);
void rte_pmd_n3k_mp_vf0_stats_dump(const struct n3k_mgmt_mp_vf0_stats_response *stats);
