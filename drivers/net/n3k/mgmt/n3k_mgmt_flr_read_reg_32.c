	...
	struct n3k_mgmt_flr_command_read_reg_32 *reg_32_read)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLR_LOG(ERR, "Failed to push reg 32 read command to cmd queue");
		return ret;
	}
	...
	uint32_t reg, uint32_t *val)
{
	...
	if (ret) {
		N3K_MGMT_FLR_LOG(ERR, "flow read 32 reg scheduling failed %d", ret);
	...
	if (reg_read_cmd->reg_32_read.error_code) {
		N3K_MGMT_FLR_LOG(ERR, "flow read 32 reg failed %d",
...
{
	if (parsed_pkt == NULL && retry_limit_reached)
		goto error_unlock_sync;

	const struct n3k_mgmt_flr_parsed_block *read_reg_block =
		&parsed_pkt->blocks[N3K_MGMT_FLR_READ_REG_32];

	if (read_reg_block->error_code && retry_limit_reached)
		goto error_unlock_sync;
	...
	memcpy_bswap32(&out_result, read_reg_block->data,
		sizeof(uint32_t));
	N3K_MGMT_REGS_LOG(DEBUG, "read_reg_32 succeeded with result: %" PRIu32,
		out_result);
