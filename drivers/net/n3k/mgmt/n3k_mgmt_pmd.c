...
alloc_pci_dev_internal(struct n3k_mgmt_pci_dev_internal **internal)
{
	...
	*internal = n3k_zmalloc("n3k_mgmt_pci_dev_internal", sizeof(**internal), 0);
	...
	if (release_ret < 0) {
		N3K_MGMT_LOG(CRIT, "mutex lock error: %d", release_ret);
		return release_ret;
	}
	int i;
...
release_lock:
	...
	if (release_ret < 0) {
		N3K_MGMT_LOG(CRIT, "mutex unlock error: %d", release_ret);
		return release_ret;
	}
	return ret;
}
		...
		struct rte_pci_device *pci_dev)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(ERR, "couldn't allocate pci dev internal, ret=%d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		N3K_MGMT_LOG(ERR, "failed to parse args, ret=%d", ret);
		goto free_internal;
	}

	ret = n3k_mgmt_vfio_setup(internal);
	if (ret < 0) {
		N3K_MGMT_LOG(ERR, "failed to setup device %s, ret=%d",
			pci_dev->name, ret);
		goto free_internal;
	}
	...
	if (ret < 0) {
		N3K_MGMT_LOG(ERR, "failed to init device %s, red=%d",
			pci_dev->name, ret);
		goto free_internal;
	}
	...
	if (ret || eth_dev == NULL) {
		N3K_MGMT_LOG(ERR, "failed to init eth device %s",
			pci_dev->name);
		goto free_internal;
	}
...
free_internal: // TODO(n3k): add proper deinit
	n3k_free(internal);
	return ret;
}
...
pop_pci_dev_internal(struct rte_pci_device *pci_dev)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(CRIT, "mutex lock error: %d", ret);
		return NULL;
	}
	for (i = 0; i < N3K_MGMT_PCI_DEV_MAX_DEV; i++)
...
release_lock:
	...
	if (ret < 0) {
		N3K_MGMT_LOG(CRIT, "mutex unlock error: %d", ret);
		return NULL;
	}
	return internal;
}
...
n3k_mgmt_pci_remove(struct rte_pci_device *pci_dev)
{
	...
	if (internal == NULL) {
		N3K_MGMT_LOG(ERR, "couldn't find pci dev internal for %s",
			pci_dev->name);
		return -EINVAL;
	}

	n3k_mgmt_mp_exit(&internal->hw);
	// n3k_mgmt_nios_exit(&internal->hw);
