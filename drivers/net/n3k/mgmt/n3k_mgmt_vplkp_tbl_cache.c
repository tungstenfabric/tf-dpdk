...
{
	N3K_MGMT_LOG(VPLKP, INFO, "Initializing vplkp tbl cache");

	hw->vplkp_cache = n3k_zmalloc(N3K_MGMT_VPLKP_TBL_CACHE_NAME,
		sizeof(*hw->vplkp_cache), 0);
	if (hw->vplkp_cache == NULL) {
		N3K_MGMT_LOG(VPLKP, INFO, "Not enough memory to init");
		return -ENOMEM;
	}
	...
	if (hw->vplkp_cache->label_to_data == NULL) {
		...
		N3K_MGMT_LOG(VPLKP, INFO, "Failed to init hash table");
		return -ENOMEM;
	}
	...
	struct n3k_mgmt_vplkp_tbl_cache_iterator *iterator)
{
	...
	if (rc < 0) {
		N3K_MGMT_LOG(VPLKP, DEBUG, "Entry not found");
		return rc;
	}

	N3K_MGMT_LOG(VPLKP, DEBUG, "Found vplkp entry: label=%" PRIu32, *mpls_label);
	...
	uint32_t mpls_label, const struct n3k_mgmt_vplkp_tbl_res *res)
{
	...
	N3K_MGMT_LOG(VPLKP, DEBUG, "Entry not found in cache. Adding new vplkp entry.");

	if (rte_atomic32_read(&hw->vplkp_cache->total_entries) == N3K_MGMT_VPLKP_TBL_ENTRIES) {
		rc = -ENOSPC;
		N3K_MGMT_LOG(VPLKP, WARNING, "No free entry in cache");
		goto release_lock;
	}
	...
	if (rc < 0) {
		N3K_MGMT_LOG(VPLKP, ERR, "Error while adding entry to cache");
		goto free_val;
	}

	rc = n3k_mgmt_vplkp_tbl_insert(hw, mpls_label, res);
	if (rc < 0) {
		N3K_MGMT_LOG(VPLKP, ERR, "Error while adding vplkp entry");
...
{
	struct _res_with_ref_count *val = NULL;
	int rc;
