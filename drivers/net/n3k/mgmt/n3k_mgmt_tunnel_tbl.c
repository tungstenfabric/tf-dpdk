	...
	const struct n3k_mgmt_tunnel_tbl_entry *entry)
{
	...
	case N3K_MGMT_TUNNEL_TYPE_VXLAN:
		...
		// regs->data_12.field.encap_vxlan_tci_1_en = entry->vxlan.tci_1_enable;
		// regs->data_12.field.encap_vxlan_tci_0_en = entry->vxlan.tci_0_enable;
		// regs->data_13.field.encap_vxlan_flags_0 = BITS_RANGE32(entry->vxlan.flags, 0, 1);
		// regs->data_14.field.encap_vxlan_flags_1 = BITS_RANGE32(entry->vxlan.flags, 1, 7);
		break;
	}
}
	...
	const struct tunnel_tbl_data_regs *regs)
{
	...
	if (regs->data_14.field.encap_vxlan_en) {
		...
		// TODO(n3k): implement
		// entry->vxlan.vni = regs->data_9.field.encap_vxlan_vni_0 |
		// 		   (regs->data_10.field.encap_vxlan_vni_1 << 11);
		// entry->vxlan.tci_1_enable = regs->data_12.field.encap_vxlan_tci_1_en;
		// entry->vxlan.tci_0_enable = regs->data_12.field.encap_vxlan_tci_0_en;
		// entry->vxlan.flags = regs->data_13.field.encap_vxlan_flags_0 |
		// 		     (regs->data_14.field.encap_vxlan_flags_1 << 1);
