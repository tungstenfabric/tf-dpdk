	...
	const struct n3k_mgmt_tunnel_tbl_entry *entry)
{
	...
	case N3K_MGMT_TUNNEL_TYPE_MPLSOGRE:
		...
		// TODO(n3k): implement along with the rest of the MPLSoGRE functionality
		break;
	...
	const struct n3k_mgmt_tunnel_tbl_entry *entry)
{
	...
	if (tunnel_idx >= (1 << MKS_VPORTLKUP_TUNNEL_TABLE_TUNNEL_TBL_CMD_ADDR_TUNNEL_TBL_ADDR_WIDTH)) {
		N3K_MGMT_LOG(TUNNEL, ERR, "Invalid tunnel_id=%" PRIu32, tunnel_idx);
		return -EINVAL;
	}
	...
	if (N3K_MGMT_SHOULD_LOG(REGS, DEBUG)) {
		N3K_MGMT_LOG(REGS, DEBUG, "Adding tunnel: tunnel_id=%" PRIu16 " regs", tunnel_idx);
		n3k_mgmt_dump_reg_tbl(N3K_MGMT_REG_TBL_TUNNEL_TBL_DATA, &tunnel_regs);
	}

	if (N3K_MGMT_SHOULD_LOG(TUNNEL, DEBUG)) {
		N3K_MGMT_LOG(TUNNEL, DEBUG, "Adding tunnel: tunnel_id=%" PRIu16, tunnel_idx);
	...
	struct n3k_mgmt_tunnel_tbl_entry *entry)
{
	...
	if (N3K_MGMT_SHOULD_LOG(TUNNEL, DEBUG)) {
		N3K_MGMT_LOG(TUNNEL, DEBUG, "Read tunnel: tunnel_id=%" PRIu32, tunnel_idx);
		n3k_mgmt_tunnel_tbl_entry_dump(entry);
	}
