	...
	uint32_t *hw_flow_id)
{
	...
	if (ret < 0) {
		n3k_mgmt_unlock(hw);
		N3K_MGMT_FLOW_LOG(WARNING, "Failed to get free flow handle");
		return ret;
	}


	if (N3K_MGMT_REGS_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_REGS_LOG(DEBUG, "Adding flow(regs)");
	...
	struct n3k_mgmt_flow_tbl_handle **handle)
{
	...
	N3K_MGMT_FLOW_LOG(DEBUG, "Adding Mirror flow");
	...
	N3K_MGMT_FLOW_LOG(INFO, "New mirror flow added successfully: flow_id=%" PRIu32,
		local_flow_id);
...
set_status_to_deleted:
	{
		__rte_unused int change_ret = n3k_mgmt_flow_tbl_change_flow_status(*handle,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_ADDING,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_DELETED);
	...
	struct n3k_mgmt_flow_tbl_handle *mirror_handle)
{
	...
	if (N3K_MGMT_FLOW_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_FLOW_LOG(DEBUG, "Adding flow");
		n3k_mgmt_flow_tbl_entry_dump(entry);
	}

	if (!entry->key.use_l2_key && !entry->key.use_tunnel_key) {
		N3K_MGMT_FLOW_LOG(ERR, "L3 key is supported only with tunnel keys.");
		return -EINVAL;
	}
	...
	N3K_MGMT_FLOW_LOG(INFO, "New flow added successfully: flow_id=%" PRIu32,
		local_flow_id);
...
disable_flow_tracker:
	{
		...
		if (ret_ft < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Couldn't disable flow tracker. ret = %d",
				ret_ft);
		}
	}
set_status_to_deleted:
	...
	struct n3k_mgmt_flow_tbl_handle **handle)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(DEBUG, "Failed to allocate handle");
		return ret;
	}
	...
	if (entry->action.use_mirroring) {
		...
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(DEBUG, "Failed offload mirror flow");
...
free_mirror_flow:
	...
	if (free_ret < 0) {
		N3K_MGMT_FLOW_LOG(DEBUG, "Failed to free mirror flow");
		goto free_flow_id;
	}
	...
	struct n3k_mgmt_flow_tbl_entry *entry)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Invalid key: flow_id=%" PRIu32, handle->flow_id);
		return ret;
	}

	ret = n3k_mgmt_flow_tbl_fill_res_info(hw, encap_type, &entry->action, &res_regs);
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Invalid action: flow_id=%" PRIu32, handle->flow_id);
		return ret;
	}
	...
	if (handle->mirror_flow) {
		...
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Failed filling action mirror info: flow_id=%" PRIu32, handle->flow_id);
			return ret;
		}
	}

	if (N3K_MGMT_FLOW_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_FLOW_LOG(DEBUG, "Read flow: flow_id=%" PRIu32, handle->flow_id);
		n3k_mgmt_flow_tbl_entry_dump(entry);
	}
	...
	struct n3k_mgmt_flow_tbl_handle *handle)
{
	...
	N3K_MGMT_FLOW_LOG(INFO, "Removing flow: flow_id=%" PRIu32, handle->flow_id);

	if (handle->flow_id >= n3k_mgmt_flow_tbl_get_max_flow_id(hw)) {
		N3K_MGMT_FLOW_LOG(ERR, "Invalid flow_id=%" PRIu32, handle->flow_id);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Flow has invalid status");
		return ret;
	}
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Couldn't delete flow from flow table: "
	...
	if (!handle->is_mirror_flow) {
		...
		if (ret_ft < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Couldn't disable flow tracker: "
	...
	struct n3k_mgmt_flow_tbl_handle *handle)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "flow_tbl_del_single_flow failed with ret=%d\n", ret);
	}

	if (mirror_handle) {
		int free_ret =
		    flow_tbl_del_single_flow(hw, mirror_handle);
		if(free_ret < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Failed to remove mirror flow.");
			return ret;
		}
