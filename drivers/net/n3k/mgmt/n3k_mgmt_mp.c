...
static int
n3k_mgmt_mp_parse_msg(const struct rte_mp_msg *msg,
	enum n3k_mgmt_mp_request_table *request_table, struct n3k_mgmt_hw **hw)
{
	struct n3k_mgmt_mp_request req; // TODO(n3k): check if size is valid
	struct rte_eth_dev *eth_dev;

	memcpy(&req, msg->param, sizeof(req));
	req.device_name[sizeof(req.device_name) - 1] = 0;
	*request_table = req.table;
...
static int
n3k_mgmt_mp_parse_dump_register_msg(const struct rte_mp_msg *msg,
	struct n3k_mgmt_mp_register_request *req, struct n3k_mgmt_hw **hw)
{
	struct rte_eth_dev *eth_dev;
...
static int
n3k_mgmt_mp_parse_vf0_stats_msg(const struct rte_mp_msg *msg,
	struct n3k_mgmt_hw **hw)
{
	struct rte_eth_dev *eth_dev;
	char device_name[N3K_MGMT_MP_DEVICE_MAX_LEN];

	memcpy(device_name, msg->param, sizeof(device_name) - 1);
...
/* Total entries number request */

static struct n3k_mgmt_mp_total_entries_response *
n3k_mgmt_mp_setup_total_entries_response(struct rte_mp_msg *response)
{
	memset(response, 0, sizeof(*response));
	strncpy(&response->name[0], N3K_MGMT_MP_REQUEST_TOTAL_ENTRIES, RTE_MP_MAX_NAME_LEN);
	response->len_param = sizeof(struct n3k_mgmt_mp_total_entries_response);
	return (struct n3k_mgmt_mp_total_entries_response *)(&response->param[0]);
}

static struct n3k_mgmt_mp_dump_register_response *
n3k_mgmt_mp_setup_register_response(struct rte_mp_msg *response)
{
	memset(response, 0, sizeof(*response));
	strncpy(&response->name[0], N3K_MGMT_MP_REQUEST_REGISTER, RTE_MP_MAX_NAME_LEN);
	response->len_param = sizeof(struct n3k_mgmt_mp_dump_register_response);
	return (struct n3k_mgmt_mp_dump_register_response *)(&response->param[0]);
}

static struct n3k_mgmt_mp_vf0_stats_response *
n3k_mgmt_mp_setup_vf0_stats_response(struct rte_mp_msg *response)
{
	memset(response, 0, sizeof(*response));
	strncpy(&response->name[0], N3K_MGMT_MP_REQUEST_VF0_STATS, RTE_MP_MAX_NAME_LEN);
	response->len_param = sizeof(struct n3k_mgmt_mp_vf0_stats_response);
	return (struct n3k_mgmt_mp_vf0_stats_response *)(&response->param[0]);
}
...
n3k_mgmt_mp_do_register(const struct rte_mp_msg *msg, const void *peer)
{
	...
	struct n3k_mgmt_mp_dump_register_response *register_response;
	struct n3k_mgmt_mp_register_request dump_request;
	struct n3k_mgmt_hw *hw;
	int ret;

	N3K_MGMT_MP_LOG(INFO, "received '%s' from secondary process",
		N3K_MGMT_MP_REQUEST_REGISTER);

	ret = n3k_mgmt_mp_parse_dump_register_msg(msg, &dump_request, &hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "specified device name is invalid");
		return ret;
	}
	...
	N3K_MGMT_MP_LOG(
		...
		dump_request.base, dump_request.offset);

	register_response = n3k_mgmt_mp_setup_register_response(&response);

	ret = n3k_mgmt_mp_fill_register_response(hw, &dump_request,
		&register_response->reg_value);
	if (ret) {
		N3K_MGMT_MP_LOG(ERR,
	...
	if (ret < 0) {
		...
		N3K_MGMT_MP_LOG(INFO, "response sent to secondary process");
	}
...
{
	struct rte_mp_msg response;
	struct n3k_mgmt_mp_vf0_stats_response *vf0_stats;
	struct n3k_mgmt_hw *hw;
	int ret;
	...
	ret = n3k_mgmt_mp_parse_vf0_stats_msg(msg, &hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "specified device name is invalid");
		return ret;
	}

	vf0_stats = n3k_mgmt_mp_setup_vf0_stats_response(&response);

	n3k_mgmt_mp_fill_vf0_stats_response(hw, vf0_stats);
...
{
	struct rte_mp_msg response;
	struct n3k_mgmt_mp_total_entries_response *total_entries_response;
	enum n3k_mgmt_mp_request_table request_table;
	struct n3k_mgmt_hw *hw;
	int ret;
	...
	ret = n3k_mgmt_mp_parse_msg(msg, &request_table, &hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "specified device name is invalid");
		return ret;
	}

	N3K_MGMT_MP_LOG(INFO, "requested table '%s'", n3k_mgmt_mp_request_type_to_str(request_table));

	total_entries_response = n3k_mgmt_mp_setup_total_entries_response(&response);

	ret = n3k_mgmt_mp_fill_total_entries_response(hw, request_table, total_entries_response);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "error occurred while extracting number of entries for table '%s'",
			n3k_mgmt_mp_request_type_to_str(request_table));
		return ret;
	}
	...
	enum n3k_mgmt_mp_request_table type)
{
	...
	while (ret != -ENOENT) {
		...
		if (ret < 0) {
			N3K_MGMT_MP_LOG(ERR, "error while filling response entry; ret = %d", ret);
		...
		if (ret < 0) {
			N3K_MGMT_MP_LOG(WARNING, "error while pushing response; ret = %d", ret);
			break;
		}
	...
	if (ret < 0)
		N3K_MGMT_MP_LOG(WARNING, "error while pushing end response; ret = %d", ret);
}
...
n3k_mgmt_mp_send_entries_service_callback(void *args)
{
	...
	N3K_MGMT_MP_LOG(DEBUG, "service callback started");
	...
	N3K_MGMT_MP_LOG(DEBUG, "runstate set to 0 for service component; service_id = %d",
...
n3k_mgmt_mp_do_entries(const struct rte_mp_msg *msg, __rte_unused const void *peer)
{
	...
	N3K_MGMT_MP_LOG(INFO, "received '%s' from secondary process", N3K_MGMT_MP_REQUEST_ENTRIES);

	ret = n3k_mgmt_mp_parse_msg(msg, &request_table, &hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "invalid request");
		return ret;
	}
	...
	hw->mp_internal->request_table = request_table;

	rte_service_component_runstate_set(hw->mp_internal->service_id, 1);
	N3K_MGMT_MP_LOG(DEBUG, "runstate set to 1 for service component; service_id = %d",
		hw->mp_internal->service_id);
...
n3k_mgmt_mp_setup_service(struct n3k_mgmt_hw *hw)
{
	...
	spec.capabilities = 0;
	spec.socket_id = 0; /* TODO(n3k): Set properly */

	ret = rte_service_component_register(&spec, &hw->mp_internal->service_id);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "failed to register service for n3k_mgmt_mp module; ret = %d", ret);
		return ret;
	}
	N3K_MGMT_MP_LOG(DEBUG,
...
n3k_mgmt_mp_cleanup_service(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_MP_LOG(DEBUG, "unregistered service callback for n3k_mgmt_mp module");
}
...
n3k_mgmt_mp_setup_rings(struct n3k_mgmt_hw *hw)
{
	...
	if (entries_ring == NULL) {
		N3K_MGMT_MP_LOG(ERR, "failed to allocate rte_ring");
		return -rte_errno;
	}
...
n3k_mgmt_mp_setup_mempool(struct n3k_mgmt_hw *hw)
{
	...
	if (mempool == NULL) {
		N3K_MGMT_MP_LOG(ERR, "failed to allocate rte_mempool; err: %s",
...
n3k_mgmt_mp_init(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_MP_LOG(INFO, "Initializing mp");
	...
	if (hw->mp_internal == NULL) {
		N3K_MGMT_MP_LOG(ERR, "failed to allocate memory for n3k_mgmt_mp internal data");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "failed to setup rings in n3k_mgmt_mp module");
		goto exit_cleanup_internal_data;
	}

	ret = n3k_mgmt_mp_setup_mempool(hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "failed to setup mempools in n3k_mgmt_mp module");
		goto exit_cleanup_rings;
	}

	ret = n3k_mgmt_mp_setup_service(hw);
	if (ret < 0) {
		N3K_MGMT_MP_LOG(ERR, "failed to setup service core for n3k_mgmt_mp module");
	...
	N3K_MGMT_MP_LOG(DEBUG, "registered '%s' MP action", N3K_MGMT_MP_REQUEST_REGISTER);
	...
	N3K_MGMT_MP_LOG(DEBUG, "registered '%s' MP action", N3K_MGMT_MP_REQUEST_TOTAL_ENTRIES);
	...
	N3K_MGMT_MP_LOG(DEBUG, "registered '%s' MP action", N3K_MGMT_MP_REQUEST_ENTRIES);
...
{
	N3K_MGMT_MP_LOG(INFO, "Deinitializing mp");
