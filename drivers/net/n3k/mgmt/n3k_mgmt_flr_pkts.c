	...
	enum n3k_mgmt_flr_block_header_cmd_type cmd_type)
{
	...
	// TODO(n3k): maybe return error instead of assert?
	...
	const void *regs, size_t regs_size)
{
	...
	if (added_data_ptr == NULL) {
		N3K_MGMT_LOG(FLR_PKTS, ERR, "Error in resizing packet");
		return -EINVAL;
	}
...
n3k_mgmt_flr_add_icv(struct rte_mbuf *pkt)
{
	...
	N3K_MGMT_LOG(FLR_PKTS, DEBUG, "Adding icv to packet");
	...
	// The icv data must be present but its contents are irrelevant as of now
	// TODO(n3k): insert real icv data when the hardware supports it
...
n3k_mgmt_flr_init_pkt(struct rte_mbuf *pkt, uint16_t pkt_id)
{
	...
	N3K_MGMT_LOG(FLR_PKTS, DEBUG, "Initializing new packet: pkt_id=%" PRIu16, pkt_id);
	...
	struct n3k_mgmt_flr_parsed_pkt *parsed_pkt)
{
	...
	for (i = 0; i < parsed_pkt->blocks_num; ++i) {
		...
		if (block_off + sizeof(*block_hdr) > pkt_size) {
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid packet size.");
			return -EINVAL;
		}
		...
		if (next_block_off > pkt_size ||
			...
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid packet or block size.");
			return -EINVAL;
		}

		if (parsed_pkt->blocks[i].start_reg_addr % 4) {
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Start register address not aligned to 4.");
			return -EINVAL;
		}

		if (interface_id != get_interface_id(parsed_pkt->blocks[i].start_reg_addr, end_reg_addr)) {
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid interface id or address range.");
			return -EINVAL;
		}
		...
		if (parsed_pkt->blocks[i].cmd_type != N3K_MGMT_FLR_BLOCK_HEADER_CMD_TYPE_READ &&
			...
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid command type.");
			return -EINVAL;
		}

		if (parsed_pkt->blocks[i].burst_cnt * 4 + sizeof(*block_hdr) != block_length) {
			N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid packet data size.");
			return -EINVAL;
		}
	...
	if (block_off - hdrs_size != payload_len) {
		N3K_MGMT_LOG(FLR_PKTS, ERR, "Invalid sum of blocks size.");
		return -EINVAL;
	}
...
bool
n3k_mgmt_flr_contains_error_block(const struct n3k_mgmt_flr_parsed_block *blocks,
	size_t blocks_num)
{
	size_t i;
	for (i = 0; i < blocks_num; ++i)
		if (blocks[i].error_code)
			return true;
	return false;
}
...
bool n3k_mgmt_flr_is_config_pkt(const struct rte_mbuf *pkt)
{
	...
	if (pkt->data_len < sizeof(struct rte_ether_hdr *))
		return false;
	eth_hdr = (const struct rte_ether_hdr *)((const uint8_t *)pkt->buf_addr + pkt->data_off);
	return eth_hdr->ether_type == rte_cpu_to_be_16(N3K_MGMT_FLR_CONFIG_HEADER_ETHER_TYPE);
}
