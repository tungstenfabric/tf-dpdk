...
struct n3k_mgmt_flow_tbl_handle {
	...
	// take read lock when needed a guarantee that the flow's status won't change
	// take write lock when needed to change the flow's status
	rte_rwlock_t lock;
...
int n3k_mgmt_flow_tbl_change_flow_status(
