	...
	for (wc = N3K_DEL_RULES_DATA_HDR_SIZE;
		...
		for (bc = 0; bc < 32; bc++) {
			if (data[wc] & (1 << bc)) {
				...
				N3K_MGMT_LOG(NIOS, DEBUG, "freed handle %" PRIu32, handle);
				processed++;
			}
		}
	}
...
n3k_mgmt_nios_free_recirc_ids(uint32_t* recirc_id, uint32_t count)
{
	...
	for (i = 0; i < count; i++) {
		/* not used */
		N3K_MGMT_LOG(NIOS, DEBUG, "Freeing recirc_id:%u\n", recirc_id[i]);
	}
}
...
n3k_mgmt_nios_handler_run(void* args)
{
	...
	if (len) {
		...
		case N3K_MSG_FREE_RECIRC: {
			N3K_MGMT_LOG(NIOS, DEBUG, "N3K_MSG_FREE_RECIRC");
			n3k_mgmt_nios_free_recirc_ids(msg->data, count);
		} break;
		case N3K_MSG_FREE_HNDL: {
			N3K_MGMT_LOG(NIOS, DEBUG, "N3K_MSG_FREE_HNDL");
			n3k_mgmt_nios_free_handles(msg->data, count);
		} break;
		default:
			N3K_MGMT_LOG(NIOS, DEBUG, "Received invalid message");
			break;
		}
	}
...
n3k_mgmt_nios_setup_service(struct n3k_mgmt_hw *hw) {
	...
	if (ret != 0) {
		N3K_MGMT_LOG(NIOS, ERR, "failed to register component");
		return ret;
	}
...
n3k_mgmt_nios_handler_register_and_run(struct n3k_mgmt_hw *hw)
{
	...
	if (hw->nios_internal == NULL) {
		N3K_MGMT_LOG(NIOS, ERR, "failed to allocate memory for n3k_mgmt_nios_shm internal data");
		return -ENOMEM;
	}
