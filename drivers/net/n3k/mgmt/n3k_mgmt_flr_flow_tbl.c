	...
	struct n3k_mgmt_flr_command_flow_create *flow_create_cmd)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "n3k_mgmt_flr_flow_get_flow_id failed: ret=%d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Failed to allocate recirc id");
		goto free_flow_id;
	}
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Failed to create flow handle");
		goto free_recirc_id;
	}

	ret = n3k_mgmt_flow_tbl_change_flow_status(flow_create_cmd->mirror_handle,
	...
	struct n3k_mgmt_flow_tbl_handle **handle)
{
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Failed to init flow handle");
		goto free_flow_id;
	}
	*handle = flow_create_cmd->handle;
	...
	if (mirror) {
		...
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(DEBUG, "n3k_mgmt_flr_flow_tbl_init_mirror_flow failed ret=%d", ret);
			goto free_flow_id;
		}
	}

	ret = n3k_mgmt_flow_tbl_change_flow_status(flow_create_cmd->handle,
		N3K_MGMT_FLOW_TBL_FLOW_STATUS_DELETED,
		N3K_MGMT_FLOW_TBL_FLOW_STATUS_ADDING);
	RTE_ASSERT(ret == 0);

	if (N3K_MGMT_FLOW_SHOULD_LOG(DEBUG)) {
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Failed to push flow create command to cmd queue");
		goto free_mirror;
	}
	...
	struct n3k_mgmt_flr_command_flow_create *flow_create_cmd)
{
	...
	case FLOW_CREATE_CMD_FLOW_NOT_ADDED:
		ret = n3k_mgmt_flow_tbl_change_flow_status(cur_handle,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_ADDING,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_FAILED_ABSENT);
		RTE_ASSERT(ret == 0);
		break;
	...
	struct n3k_mgmt_flr_command_flow_create *flow_create_cmd)
{
	...
	if (flow_create_cmd->insert_mirror_flow) {
		N3K_MGMT_FLOW_LOG(INFO,
			"Successfully added scheduled mirror flow: flow_id=%" PRIu32,
			cur_handle->flow_id);

		// flr_flow_tbl_schedule_base_add(hw, flow_create_cmd);
		// now insert base flow
		// todo(n3k): reset cmd counter
	...
	const struct n3k_mgmt_flr_parsed_pkt *parsed_pkt)
{
	...
	if (!mirror) {
		const struct n3k_mgmt_flr_parsed_block *ft_cmd_block =
			&parsed_pkt->blocks[N3K_MGMT_FLR_FLOW_CREATE_FLOW_TRACKER_COMMAND_BLOCK];
...
{
	int ret = flr_flow_tbl_add_parse_error_type(flow_create_cmd, parsed_pkt);
	if (ret == 0) {
	...
	struct rte_mbuf *pkt)
{
	...
	// TODO(n3k): use enum n3k_mgmt_flr_add_flow_create_blocks_order order
	...
	struct n3k_mgmt_flr_command_flow_destroy *flow_destroy_cmd)
{
	...
	case FLOW_DESTROY_CMD_FLOW_NOT_DESTROYED:
		ret = n3k_mgmt_flow_tbl_change_flow_status(handle,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_DELETING,
			N3K_MGMT_FLOW_TBL_FLOW_STATUS_FAILED_PRESENT);
		RTE_ASSERT(ret == 0);
		break;
	...
	struct n3k_mgmt_flr_command_flow_destroy *flow_destroy_cmd)
{
	...
	case DELETE_ACTION_DEFAULT:
		...
		ret = n3k_mgmt_flr_cmd_queue_push_command(hw, container_of(flow_destroy_cmd,
			struct n3k_mgmt_flr_command, flow_destroy));
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Failed to push flow destroy command to cmd queue");
			flow_destroy_cmd->flow_destroyed = FLOW_DESTROY_CMD_FLOW_NOT_DESTROYED;
...
n3k_mgmt_flr_flow_tbl_del_handle_response(struct n3k_mgmt_hw *hw,
	...
	bool retry_limit_reached)
{
	int ret = flr_flow_tbl_del_parse_error_type(flow_destroy_cmd, parsed_pkt);
	if (ret == 0) {
	...
	struct rte_mbuf *pkt)
{
	...
	// TODO(n3k): use enum n3k_mgmt_flr_add_flow_destroy_blocks_order order
	//            when adding new blocks
	...
	struct n3k_mgmt_flow_tbl_entry *tbl_entry)
{
	...
	enum n3k_mgmt_flow_tbl_flow_status status;
	while (true) { // TODO(n3k): move this waiting to separate function
		rte_rwlock_read_lock(&handle->lock);
		if (handle->status != N3K_MGMT_FLOW_TBL_FLOW_STATUS_ADDING) {
			status = handle->status;
			rte_rwlock_read_unlock(&handle->lock);
	...
	if (ret < 0) {
		N3K_MGMT_FLOW_LOG(ERR, "Failed to push flow dump command to cmd queue");
	...
	bool retry_limit_reached)
{
	...
	const struct n3k_mgmt_flr_parsed_block* key_block =
		&parsed_pkt->blocks[N3K_MGMT_FLR_FLOW_DUMP_FLOW_READ_KEY_BLOCK];
	const struct n3k_mgmt_flr_parsed_block* res_block =
		&parsed_pkt->blocks[N3K_MGMT_FLR_FLOW_DUMP_FLOW_READ_RES_BLOCK];
	...
	if (retry_limit_reached && parsed_pkt->error) {
		ret = -EIO;
		goto unlock_sync;
	}
	...
	if (flow_dump_cmd->get_mirror_flow) {
		...
		ret = n3k_mgmt_flow_tbl_fill_key_info(hw, &flow_dump_cmd->out_entry->key,
			&key_regs);
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Failed to fill flow key info; ret=%d", ret);
			goto unlock_sync;
		}

		ret = n3k_mgmt_flow_tbl_fill_res_info(hw, N3K_MGMT_TUNNEL_TYPE_MPLSOUDP,
			&flow_dump_cmd->out_entry->action, &res_regs);
		if (ret < 0) {
			N3K_MGMT_FLOW_LOG(ERR, "Failed to fill flow res info; ret=%d", ret);
			goto unlock_sync;
		}

		if (flow_dump_cmd->handle->mirror_flow != NULL) {
			// TODO(n3k): clear cmd metadata
