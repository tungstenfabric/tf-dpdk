...
init_recirc_id_ring(struct n3k_mgmt_hw *hw)
{
	...
	*recirc_id_ring = rte_ring_create(
	...
	if (*recirc_id_ring == NULL) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to allocate rte_ring");
		return -rte_errno;
	}
	...
	for (i = 0; i < MAX_RECIRC_IDX; ++i) {
		...
		if (ret < 0) {
			N3K_MGMT_LOG(FLOW, ERR, "Failed to enqueue recirc_id to ring");
			goto free_ring;
		}
	}
...
{
	int ret;
	N3K_MGMT_LOG(FLOW, INFO, "Initializing recirc_id allocator");

	if (hw->recirc_id_alloc_internal) {
		N3K_MGMT_LOG(FLOW, ERR, "recirc_id allocator already initialized");
		return -EINVAL;
	}
	...
	if (!hw->recirc_id_alloc_internal) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to allocate recirc_id_alloc_internal");
		return -ENOMEM;
	}

	ret = init_recirc_id_ring(hw);
	if (ret < 0) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to init recirc_id ring");
		goto free_allocator;
	}
...
n3k_mgmt_recirc_id_alloc(struct n3k_mgmt_hw *hw, uint32_t *recirc_id)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to dequeue from recirc_id ring");
		return ret;
	}
