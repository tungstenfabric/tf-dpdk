...
n3k_mgmt_parse_mq_nb(const char *mq_str, void *extra_args)
{
	...
	if (args->mqs_param_count > N3K_VF_PORTS_MAX_COUNT) {
		N3K_MGMT_LOG(DEFAULT, ERR, "More MQs parameters than max number of VFs %d",
	...
	void *extra_args)
{
	...
	ret = n3k_parse_kvargs_list(value, extra_args, n3k_mgmt_parse_mq_nb);
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Parsing of mqs argument failed");
		return ret;
	}
	...
	struct n3k_mgmt_args *args)
{
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "MQs parsing for %s failed", dev_name);
		return ret;
	}
	...
	struct n3k_mgmt_args *args)
{
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "PF MQ number parsing for %s failed", dev_name);
		return ret;
	}
	...
	void *extra_args)
{
	...
	if (strcmp(value, "flr") == 0) {
		N3K_MGMT_LOG(DEFAULT, INFO, "Setting FLR insert mode");
		args->insert_type = N3K_MGMT_HW_INSERT_TYPE_FLR;
		return 0;
	} else if (strcmp(value, "flr-sim") == 0) {
		N3K_MGMT_LOG(DEFAULT, INFO, "Setting FLR-SIM insert mode");
		args->insert_type = N3K_MGMT_HW_INSERT_TYPE_FLR_SIM;
		return 0;
	} else if (strcmp(value, "csr") == 0) {
		N3K_MGMT_LOG(DEFAULT, INFO, "Setting CSR insert mode");
		args->insert_type = N3K_MGMT_HW_INSERT_TYPE_CSR;
		return 0;
	}

	N3K_MGMT_LOG(DEFAULT, ERR, "Invalid insert type %s", value);
	...
	struct n3k_mgmt_args *args)
{
	...
	if (ret == 0) {
		N3K_MGMT_LOG(DEFAULT, INFO,
			"Insert mode not specified. Setting FLR insert mode");
		return 0;
	}
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Insert type parsing for %s failed", dev_name);
		return ret;
	}
	...
	struct n3k_mgmt_args *args)
{
	...
	for (i = 0; i < RTE_DIM(prob_kvargs); ++i) {
		...
		if (ret) {
			...
			if (ret < 0) {
				N3K_MGMT_LOG(DEFAULT, ERR, "FLR sim %s parsing for %s failed",
...
static const char *
pci_device_args(const struct rte_pci_device *dev)
{
...
n3k_mgmt_args_parse(struct rte_pci_device *dev, struct n3k_mgmt_args *args)
{
	...
	name = dev->name;
	kvlist = rte_kvargs_parse(pci_device_args(dev),
		n3k_mgmt_init_valid_arguments);

	if (!kvlist) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to parse arguments for %s", name);
		return -EINVAL;
	}
