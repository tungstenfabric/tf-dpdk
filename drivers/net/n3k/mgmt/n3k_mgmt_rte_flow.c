...
struct rte_flow {
	bool ignored; // flow won't correspond to hw flow (slow path will be used)
			...
			struct rte_flow_error *error)
{
	...
	if (!n3k_mgmt_rte_flow_verify(pattern, actions))
		return rte_flow_error_set(error, EINVAL,
					  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
					  "Invalid flow");
	return rte_flow_error_set(error, 0, RTE_FLOW_ERROR_TYPE_NONE, NULL,
		"Valid flow");
}
			...
			struct rte_flow_error *error)
{
	...
	flow = n3k_malloc("n3k_mgmt_rte_flow", sizeof(*flow), 0);
	if (!flow) {
	...
	if (unlikely(n3k_mgmt_rte_flow_is_lacp_flow(pattern, actions))) {
		flow->ignored = true;
		return flow;
	}

	ret = n3k_mgmt_rte_flow_to_tables(pattern, actions, &parsed_flow);
	if (ret != 0) {
			...
			struct rte_flow_error *error)
{
	...
	ret = n3k_mgmt_rte_flow_to_tables(pattern, actions,
		&cmd->flow_create.flow);
	if (ret != 0) {
		rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
			   NULL, "converting provided pattern and actions failed");
	...
	ret = n3k_mgmt_flr_flow_schedule_add(hw, &cmd->flow_create, &flow->desc);
	if (ret) {
		rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
			...
			struct rte_flow_error *error)
{
	...
	ret = n3k_mgmt_flow_entry_description_remove(hw, &flow->desc);
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "N3K flow deletion failed");
		rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
			...
			struct rte_flow_error *error)
{
	...
	if (unlikely(flow->ignored)) {
		...
		ret = rte_pmd_n3k_flow_tracker_get(dev, flow->desc.flow_handle, &stats);
		if (ret) {
			rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
			...
			struct rte_flow_error *error)
{
	...
	if (likely(!flow->ignored)) {
		...
		n3k_mgmt_flow_tbl_get_cached_stats(flow->desc.flow_handle, &ft_stats);
