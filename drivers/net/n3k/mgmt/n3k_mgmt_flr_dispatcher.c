...
n3k_mgmt_flr_dispatcher_init(struct n3k_mgmt_hw *hw)
{
	...
	/* TODO(n3k): Set properly when the NUMA nodes functionality will
		be implemented */
	const int SOCKET_ID = 0;

	N3K_MGMT_LOG(FLR_DISPATCHER, INFO, "Initializing FLR pkt pool");

	if (hw->flr_dispatcher_internal) {
		N3K_MGMT_LOG(FLR_DISPATCHER, ERR, "FLR pkt pool already initialized");
		return -EINVAL;
	}
	...
	*mempool = rte_pktmbuf_pool_create(
	...
	if (!*mempool) {
		N3K_MGMT_LOG(FLR_DISPATCHER, ERR, "Failed to allocate rte_mempool");
		return -rte_errno;
	}
...
{
	/* TODO: Handle multiple queues for dispatching flr packets when multi-queue support will be implemented in hardware for slow-path */
	...
	for (idx = 0; idx < N3K_MGMT_FLR_COMMAND_PROCESSED_MAX_COUNT; ++idx) {
		/* TODO: Optimize: get multiple packets in a batch */
		struct n3k_mgmt_flr_command_pack pack =
			n3k_mgmt_flr_rdy_cmd_hdlr_process_cmd_pack(hw);
		...
		if (!pkt) {
			...
			N3K_MGMT_LOG(FLR_DISPATCHER, ERR, "Failed to allocate mbuf");
			break;
		}

		if (unlikely(n3k_mgmt_flr_command_pack_to_pkt(hw, &pack, pkt))) {
			/* The command is not freed here. Let it be handled by timeout. */
			N3K_MGMT_LOG(FLR_DISPATCHER, ERR, "Failed to convert command pack to packet");
			rte_pktmbuf_free(pkt);
			continue;
		}

		if (N3K_MGMT_SHOULD_LOG(FLR_DISPATCHER, DEBUG)) {
			N3K_MGMT_LOG(FLR_DISPATCHER, DEBUG,
				"Prepared packet to send from command pack of size %zu commands",
				pack.metadata.cmd_cnt);
			dump_packet("Config packet", pkt);
		}

		if (N3K_MGMT_SHOULD_LOG(FLR_DISPATCHER, INFO)) {
			N3K_MGMT_LOG(FLR_DISPATCHER, INFO,
				"Command pack ready to send");
			n3k_mgmt_flr_command_pack_dump(&pack);
		}
	...
	if (nb_sent) {
		N3K_MGMT_LOG(FLR_DISPATCHER, INFO, "Sent %" PRIu16 " FLR config packets", nb_sent);
