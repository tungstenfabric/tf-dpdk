...
static const char *
get_reg_base_str(size_t base_addr)
{
	switch (base_addr) {
...
#define LOG_REGISTER_ADDR_OP(reg_addr, value) \
	N3K_MGMT_LOG(REGS, DEBUG, \
		"reg addr: 0x%04" PRIx32 ".%04" PRIx32 ", reg val: 0x%08" PRIx32, \
		(uint32_t)(reg_addr) >> 16, (uint32_t)(reg_addr) & 0xffff, (uint32_t)value)

uint32_t
csr_read(struct n3k_mgmt_hw *hw, size_t base_module, size_t offset)
{
...
uint32_t
csr_read_reg_addr(struct n3k_mgmt_hw *hw, size_t reg_addr)
{
	char* addr = (char*)(hw->pci_resource_start);
	uint32_t val = N3K_MGMT_READ_REG32(addr + reg_addr);
	LOG_REGISTER_ADDR_OP(reg_addr, val);
	return val;
}

void
csr_write(struct n3k_mgmt_hw *hw, size_t base_module, size_t offset, uint32_t val)
{
	LOG_REGISTER_OP(base_module, offset, val);
	char* addr = (char*)(hw->pci_resource_start);
...
void
csr_write_reg_addr(struct n3k_mgmt_hw *hw, size_t reg_addr, uint32_t val)
{
	LOG_REGISTER_ADDR_OP(reg_addr, val);
	char* addr = (char*)(hw->pci_resource_start);
	N3K_MGMT_WRITE_REG32(val, addr + reg_addr);
}
