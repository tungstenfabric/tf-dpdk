...
n3k_mgmt_init_vq_alloc(u8 size)
{
	...
	if (mq_ctx->pool.free < size) {
		N3K_MGMT_LOG(DEFAULT, ERR, "not enough vqs available on pool\n");
		return NULL;
	}

	offset = n3k_mgmt_init_vq_find_free(mq_ctx, size);
	if (!offset) {
		N3K_MGMT_LOG(DEFAULT, ERR, "pool is fragmented\n");
		return NULL;
	}
...
n3k_mgmt_init_set_vq(struct n3k_mgmt_hw *hw, u8 is_pf, u8 pf, u8 vf, u8 queues)
{
	...
	if (!queues) {
		if (!vqs) {
			N3K_MGMT_LOG(DEFAULT, ERR, "given %s %u has no assigned queues.\n",
	...
	if (vqs) {
		N3K_MGMT_LOG(DEFAULT, ERR, "given %s %u has already assigned queues.\n",
			is_pf ? "pf" : "vf", is_pf ? pf : vf);
		return -EINVAL;
	}
...
n3k_mgmt_init_check_attr_range(u32 data, u32 attr)
{
	if (n3k_mgmt_init_check_range(data, MQ_RANGE[attr].min, MQ_RANGE[attr].max)) {
		N3K_MGMT_LOG(DEFAULT, ERR, "%s = %u - out of range\n",
...
n3k_mgmt_init_set_vf_mq(struct n3k_mgmt_hw *hw, u8 pf, u8 vf, u8 queues)
{
	...
	if (rc) {
		N3K_MGMT_LOG(DEFAULT, ERR, "pf id out of range: %u.\n", pf);
		return rc;
	}

	rc = n3k_mgmt_init_check_attr_range(vf, N3K_MGMT_INIT_MQ_ATTR_VF);
	if (rc) {
		N3K_MGMT_LOG(DEFAULT, ERR, "vf id out of range: %u.\n", vf);
		return rc;
	}

	rc = n3k_mgmt_init_check_attr_range(queues, N3K_MGMT_INIT_MQ_ATTR_QUEUES);
	if (rc) {
		N3K_MGMT_LOG(DEFAULT, ERR, "number of queues out of range: %u.\n", queues);
		return rc;
	}
...
n3k_mgmt_init_get_pf_mq(u8 pf, u8 *queues)
{
	...
	if (!vqs)
		N3K_MGMT_LOG(DEFAULT, DEBUG, "couldn't find queues for pf %u, assuming non-MQ mode\n", pf);
...
n3k_mgmt_init_get_vf_mq(u8 pf, u8 vf, u8 *queues)
{
	...
	if (!vqs)
		N3K_MGMT_LOG(DEFAULT, DEBUG, "couldn't find queues for pf %u, vf %u. assuming non-MQ mode\n", pf, vf);
