...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Disabling all PARS options");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Disabling PARS option: PTSUBPT");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling PARS option: MPLSoUDP");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling PARS option: MPLSoGRE");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling PARS option: Bypass EML for FIN/RST TCP packets");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Disabling PARS option: QID");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling PARS option: enableMIRROR");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling PARS option: enableNEWHDR");
...
static void n3k_mgmt_mod_stats_reset(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "Resetting MOD_STATS_CFG options");
...
{
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling MOD_STATS_CFG option: intPortRepEn");
...
static void n3k_mgmt_lkup_set_flr_mode(struct n3k_mgmt_hw *hw, bool enable)
{
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "Enabling FLR mode: enFlrIbCfgMode");
...
n3k_mgmt_init_init(struct n3k_mgmt_hw *hw, const struct n3k_mgmt_args *args)
{
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to flush the emlc table");
		return -EIO;
	}
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Error during n3k_mgmt_init_ft_init. ret = %d",
			ret);
		return -EFAULT;
	}
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "vportlkup table initalization succeeded");
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "flow tracker initalization succeeded");
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "flow table handles initalization succeeded");
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "flow id allocator initalization succeeded");
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "recirc id allocator initalization succeeded");
	...
	N3K_MGMT_LOG(DEFAULT, INFO, "handle mapping initalization succeeded");

	ret = n3k_mgmt_lag_init(hw);
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR,
			"n3k_mgmt_lag_init failed. ret = %d",
			ret);
		return ret;
	}
	N3K_MGMT_LOG(DEFAULT, INFO, "lag initalization succeeded");
		...
		hw->insert_type == N3K_MGMT_HW_INSERT_TYPE_FLR)
	{
		...
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR command queue initalization succeeded");

		ret = n3k_mgmt_flr_flow_stats_cmd_src_init(hw);
		if (ret) {
			N3K_MGMT_LOG(DEFAULT, ERR,
				"n3k_mgmt_flr_flow_stats_cmd_src_init failed. ret = %d",
				ret);
			return ret;
		}
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR flow stats command source initalization succeeded");
		...
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR dispatcher initalization succeeded");
		...
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR ACK dispatcher initalization succeeded");
		...
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR commands initalization succeeded");

		ret = n3k_mgmt_flr_rdy_cmd_hdlr_init(hw);
		if (ret) {
			N3K_MGMT_LOG(DEFAULT, ERR,
				"n3k_mgmt_flr_rdy_cmd_hdlr_init failed. ret = %d",
				ret);
			return ret;
		}
		ret = n3k_mgmt_flr_rdy_cmd_hdlr_register_src(hw,
			n3k_mgmt_flr_cmd_queue_pop);
		RTE_ASSERT(ret == 0);
		ret = n3k_mgmt_flr_rdy_cmd_hdlr_register_src(hw,
			n3k_mgmt_flr_flow_stats_cmd_src_pop);
		RTE_ASSERT(ret == 0);
		N3K_MGMT_LOG(DEFAULT, INFO, "FLR ready commands handler initalization succeeded");
	}
	if (hw->insert_type == N3K_MGMT_HW_INSERT_TYPE_FLR_SIM) {
		...
		N3K_MGMT_LOG(DEFAULT, INFO, "Flr simulator initalization succeeded");
	}
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "n3k_mgmt_tunnel_tbl_cache_init() failed %d", ret);
		return ret;
	}

	ret = n3k_mgmt_smac_tbl_cache_init(hw);
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "n3k_mgmt_smac_tbl_cache_init() failed %d", ret);
		return ret;
	}
	...
	for (i = 0; i < args->mqs_param_count; ++i) {
		...
		if (ret) {
			// TODO: free all
			N3K_MGMT_LOG(DEFAULT, ERR, "Setting MQ for vf%u failed", i);
			return ret;
		}
	}
	...
	if (args->pf_mqs > 0) {
		...
		if (ret) {
			N3K_MGMT_LOG(DEFAULT, ERR, "Setting MQ for PF failed");
			return ret;
		}
	}
