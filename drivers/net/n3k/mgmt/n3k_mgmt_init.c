...
int
ifc_init(struct n3k_mgmt_hw *hw, const struct n3k_mgmt_args *args)
{
	int ret;
	 ...
	 * here.
	 */
	ifc_notify_init_begin(hw); // TODO(n3k): what is this
	...
	ret = ifc_ft_init(hw); // TODO(n3k): move to n3k_mgmt_flow_tracker_init
	if (ret) {
		N3K_MGMT_LOG(ERR, "Error during ifc_ft_init. ret = %d",
			ret);
		return -EFAULT;
	}

	ifc_notify_init_done(hw);
	...
	// ifc_write_timeout_val_to_reg(IFC_UDP_CONNTRACK_INIT, MAX_TIMEOUT);
	// ifc_write_timeout_val_to_reg(IFC_UDP_CONNTRACK_ESTABLISHED, MAX_TIMEOUT);
