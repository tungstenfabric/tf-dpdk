...
n3k_mgmt_lock(struct n3k_mgmt_hw *hw)
{
	...
	while (1) {
		...
		if (tries % (1000000 / sleep_time) == 0) {
			N3K_MGMT_LOG(DEFAULT, WARNING, "Waiting for lock for %uus",
				(tries * sleep_time));
		}
	}
}
...
void
n3k_mgmt_unlock(struct n3k_mgmt_hw *hw)
{
	csr_write(hw, MKS_CHIP_MKS_LKUP_BASE, MKS_LKUP_SEMAPHORE_UNLOCK0, 1);
}
