...
int n3k_mgmt_action_entries_add(struct n3k_mgmt_hw *hw, struct n3k_mgmt_flow_action *action)
{
	...
	case N3K_MGMT_FLOW_TBL_ACTION_ENCAP: {
		...
		if (ret < 0) {
			N3K_MGMT_LOG(DEFAULT, ERR, "Failed to add tunnel entry: ret=%d", ret);
			return ret;
		}
	...
	struct n3k_mgmt_flow_tbl_handle **handle)
{
	...
	if (ret) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to add flow tbl entry: ret=%d", ret);
		goto remove_key;
	}
	...
	struct n3k_mgmt_flow_entry_description *desc)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to remove flow table entry: ret=%d",
			ret);
	}
	...
	const struct n3k_mgmt_flow_key_description *desc)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to remove vplkp tbl entry: ret=%d", ret);
		return ret;
	}
	return 0;
}
	...
	const struct n3k_mgmt_flow_action_description *desc)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to remove tunnel entry: ret=%d", ret);
		return ret;
	}

	ret = n3k_mgmt_smac_tbl_cache_yield(hw, desc->smac_id);
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to remove smac entry: ret=%d", ret);
		return ret;
	}
