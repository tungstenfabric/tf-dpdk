	...
	const struct rte_ether_addr *addr)
{
	...
	if (port_id >= 16) {
		N3K_MGMT_SMAC_LOG(ERR, "Invalid smac_id=%" PRIu8, port_id);
		return -EINVAL;
	}
	...
	if (N3K_MGMT_REGS_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_REGS_LOG(DEBUG, "Setting SMAC: smac_id=%" PRIu8 " regs", port_id);
	...
	if (N3K_MGMT_SMAC_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_SMAC_LOG(DEBUG, "Setting SMAC: smac_id=%" PRIu8, port_id);
	...
	N3K_MGMT_SMAC_LOG(INFO, "SMAC %" PRIu8 " set successfully", port_id);
	...
	struct rte_ether_addr *addr)
{
	...
	*addr = n3k_mgmt_convert_uint_to_rte_ether_addr(smac_addr);
	...
	if (N3K_MGMT_SMAC_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_SMAC_LOG(DEBUG, "Read SMAC: smac_id=%" PRIu8, port_id);
		n3k_mgmt_smac_tbl_addr_dump(addr);
	}
