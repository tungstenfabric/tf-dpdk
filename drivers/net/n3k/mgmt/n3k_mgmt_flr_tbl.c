	...
	struct n3k_mgmt_flow_entry_description *desc)
{
	...
	n3k_mgmt_flow_entry_to_description(NULL, &flow_create_cmd->flow, desc);

	ret = n3k_mgmt_flr_flow_tbl_schedule_add(hw, flow_create_cmd,
		&desc->flow_handle);
	if (ret) {
		N3K_MGMT_LOG(FLR_DEFAULT, ERR,
			"Failed to schedule add flow tbl entry: ret=%d", ret);
		goto remove_key;
	}
