...
int n3k_mgmt_smac_tbl_cache_next_index(struct n3k_mgmt_hw *hw, uint8_t *smac_id)
{
	...
	N3K_MGMT_LOG(SMAC, DEBUG, "Started iterating: smac_id=%" PRIu8, (uint8_t)(*smac_id + 1));
	...
	for (*smac_id = *smac_id + 1;
		...
		if (rte_atomic32_read(&cur_id_to_key->ref_count)) {
			N3K_MGMT_LOG(SMAC, DEBUG, "Found smac entry: smac_id=%" PRIu8,
				*smac_id);
			return 0;
		}
	}

	N3K_MGMT_LOG(SMAC, DEBUG, "Entry not found");
...
int n3k_mgmt_smac_tbl_cache_init(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_LOG(SMAC, INFO, "Initializing smac tbl cache");
	...
	uint8_t* smac_id)
{
	...
	if (N3K_MGMT_SHOULD_LOG(SMAC, DEBUG)) {
		N3K_MGMT_LOG(SMAC, DEBUG, "Fetching smac tbl cache entry:");
	...
	N3K_MGMT_LOG(SMAC, DEBUG, "Key not found in cache. Adding new smac.");
	...
	if (i == N3K_MGMT_SMAC_TBL_MAX_ENTRIES) {
		// all entries used
		N3K_MGMT_LOG(SMAC, WARNING, "No free entry in cache");
		rc = -ENOSPC;
		goto release_lock;
	}
	...
	*smac_id = i;
	...
	rc = rte_hash_add_key_data(hw->smac_cache->key_to_id, key_rfcnt->key,
		(void*)id_as_ptr);
	if (rc < 0) {
		N3K_MGMT_LOG(SMAC, ERR, "Error while adding entry to cache");
	...
	uint8_t* smac_id)
{
	...
	if (rc < 0) {
		if (N3K_MGMT_SHOULD_LOG(SMAC, DEBUG)) {
			N3K_MGMT_LOG(SMAC, DEBUG, "Couldn't find id for key: ret=%d", rc);
			n3k_mgmt_smac_tbl_addr_dump(key);
		}
		return rc;
	}
	...
	uint8_t smac_id, struct rte_ether_addr* out_key)
{
	...
	if (smac_id >= N3K_MGMT_SMAC_TBL_MAX_ENTRIES) {
		N3K_MGMT_LOG(SMAC, DEBUG, "Invalid smac_id=%" PRIu8, smac_id);
		return -ENOENT;
	}
	...
	if (N3K_MGMT_SHOULD_LOG(SMAC, DEBUG)) {
		N3K_MGMT_LOG(SMAC, DEBUG, "Found entry by id: smac_id=%" PRIu8, smac_id);
...
int n3k_mgmt_smac_tbl_cache_yield(struct n3k_mgmt_hw *hw, uint8_t smac_id)
{
	...
	if (smac_id >= N3K_MGMT_SMAC_TBL_MAX_ENTRIES) {
		N3K_MGMT_LOG(SMAC, ERR, "Invalid smac_id=%" PRIu8, smac_id);
		return -ENOENT;
	}
