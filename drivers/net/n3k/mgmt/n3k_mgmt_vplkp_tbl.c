...
#include "n3k_mgmt_vplkp_tbl_regs_api.h"
...
int n3k_mgmt_vplkp_tbl_init(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_VPLKP_LOG(INFO, "Initializing vplkp tbl");

	ret = n3k_mgmt_poll_ctrl_reg(hw, N3K_MGMT_CTRL_REG_VPLKP_EMLC_GEN_CTRL);
	if (ret != 0) {
		N3K_MGMT_VPLKP_LOG(ERR, "Failed to initialize vplkp tbl");
		return ret;
	}

	ret = n3k_mgmt_vplkp_tbl_flush(hw);
	if (ret != 0) {
		N3K_MGMT_VPLKP_LOG(ERR, "Failed to flush vplkp tbl");
		return ret;
	}
	return 0;
}
...
int n3k_mgmt_vplkp_tbl_flush(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_VPLKP_LOG(INFO, "Flushing vplkp table");
	...
	const struct n3k_mgmt_vplkp_tbl_res *res)
{
	...
	N3K_MGMT_VPLKP_LOG(INFO, "Adding vplkp entry: label=%" PRIu32, key);
	if (N3K_MGMT_VPLKP_SHOULD_LOG(DEBUG))
		n3k_mgmt_vplkp_tbl_res_dump(res);

	if (key >= (1 << 20)) {
		N3K_MGMT_VPLKP_LOG(ERR, "Invalid vplkp label: label=%" PRIu32, key);
		return -EINVAL;
	}
	if (res->keymask >= 8 || res->vport >= 4) {
		N3K_MGMT_VPLKP_LOG(ERR, "Invalid vplkp result: label=%" PRIu32, key);
		return -EINVAL;
	}
	...
	if (N3K_MGMT_REGS_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_REGS_LOG(DEBUG, "Adding vplkp entry: regs");
...
int n3k_mgmt_vplkp_tbl_delete(struct n3k_mgmt_hw *hw, uint32_t key)
{
	...
	N3K_MGMT_VPLKP_LOG(INFO, "Deleting vplkp entry: label=%" PRIu32, key);
	...
	if (N3K_MGMT_REGS_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_REGS_LOG(DEBUG, "Deleting vplkp entry: regs");
		printf("MKS_VPORTLKUP_TUNNEL_TABLE_VPLKP_EMLC_KEY_0_t = 0x%.8" PRIx32 ", 0b%s\n",
			key_reg.val, n3k_mgmt_convert_uint_to_bits_str(key_reg.val, 32));
	}
	...
	struct n3k_mgmt_vplkp_tbl_res *res)
{
	...
	N3K_MGMT_VPLKP_LOG(DEBUG, "Searching vplkp entry: label=%" PRIu32, key);
	...
	if (N3K_MGMT_VPLKP_SHOULD_LOG(DEBUG)) {
		N3K_MGMT_VPLKP_LOG(DEBUG, "Found vplkp entry: label=%" PRIu32, key);
		n3k_mgmt_vplkp_tbl_res_dump(res);
	}
