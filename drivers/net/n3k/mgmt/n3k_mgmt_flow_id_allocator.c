...
init_flow_id_ring(struct n3k_mgmt_hw *hw)
{
	...
	*flow_id_ring = rte_ring_create(
	...
	for (i = min_handle; i < max_handles; ++i) {
		...
		if (ret < 0) {
			N3K_MGMT_LOG(DEFAULT, ERR, "Failed to enqueue flow_id to ring");
			goto free_ring;
		}
	}
...
free_ring:
	// TODO(n3k): free ids to hw when software id management is implemented
...
{
	int ret;
	N3K_MGMT_LOG(DEFAULT, INFO, "Initializing flow_id allocator");

	if (hw->flow_id_alloc_internal) {
		N3K_MGMT_LOG(DEFAULT, ERR, "flow_id allocator already initialized");
		return -EINVAL;
	}
	...
	if (!hw->flow_id_alloc_internal) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to allocate flow_id_alloc_internal");
		return -ENOMEM;
	}

	ret = init_flow_id_ring(hw);
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to init flow_id ring");
		goto free_allocator;
	}
...
n3k_mgmt_flow_id_alloc(struct n3k_mgmt_hw *hw, uint32_t *handle)
{
	...
	if (ret < 0) {
		N3K_MGMT_LOG(DEFAULT, ERR, "Failed to dequeue from flow_id ring");
		return ret;
	}
	*handle = val_as_ptr;
