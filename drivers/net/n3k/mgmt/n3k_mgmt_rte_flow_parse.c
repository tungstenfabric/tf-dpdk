	...
	const struct rte_flow_action *action, struct n3k_mgmt_flow_entry *flow)
{
	...
	if (flow->key.key_raw.use_l2_key) {
		N3K_MGMT_LOG(ERR, "Cannot add inner eth heder when already present");
		return -EINVAL;
	}

	if (action_raw_encap->preserve != NULL) {
		N3K_MGMT_LOG(ERR, "Bit masks to preserve are not supported");
		return -ENOTSUP;
	}
	...
	const struct rte_flow_action *action, struct n3k_mgmt_flow_entry *flow)
{
	...
	if (data == NULL) {
		N3K_MGMT_LOG(ERR, "Encap data ptr is not set");
		return -EINVAL;
	}
	...
	if ((sizeof(*eth_hdr) + sizeof(*ipv4_hdr) +
		...
		N3K_MGMT_LOG(ERR, "Invalid encap data length");
		return -EINVAL;
	}
...
{
	if (!flow->key.key_raw.use_l2_key) {
		N3K_MGMT_LOG(ERR, "Eth encap missing. Cannot handle L3 packet");
		return -EINVAL;
	}
	...
	enum n3k_mgmt_parser_flow_type flow_type)
{
	...
	N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "pattern flow type %s", n3k_mgmt_parser_flow_type_names[flow_type]);

	for (pos = 0; state != PATTERN_STATE_END; pos++) {
		enum rte_flow_item_type type = patterns[pos].type;
		N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "pattern pos %zu", pos);
		if (type >= PATTERN_MAX_VALUE) {
			N3K_MGMT_LOG(ERR, "Invalid pattern type %d", type);
			return -EINVAL;
		}
	...
	N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "pattern parsing ok");
	return 0;
}
	...
	enum n3k_mgmt_parser_flow_type flow_type)
{
	...
	N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "action flow_type %s", n3k_mgmt_parser_flow_type_names[flow_type]);

	for (pos = 0; state != ACTION_STATE_END; pos++) {
		N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "action pos %zu", pos);
		enum rte_flow_action_type type = actions[pos].type;
		if (type >= ACTION_MAX_VALUE) {
			N3K_MGMT_LOG(ERR, "Invalid action type %d", type);
			return -EINVAL;
		}
	...
	N3K_MGMT_FLOW_PARSE_LOG(DEBUG, "action parsing ok");
	return 0;
}
...
get_target_action_queue_num(struct n3k_mgmt_flow_entry* parsed_flow,
	...
	case N3K_VDEV_DEV_TYPE_LAG:
		ifc_get_pf_mq(dev_id->id, (u8*) queues);
		return 0;
	case N3K_VDEV_DEV_TYPE_VF:
		ifc_get_vf_mq(0, dev_id->id, (u8*) queues);
		return 0;
	}
	return -EINVAL;
}
...
{
	struct n3k_mgmt_flow_hash_data hash_data;
	uint32_t hash;

	hash_data.daddr = parsed_flow->key.key_raw.l3.ipv4.dst_ip;
	hash_data.saddr = parsed_flow->key.key_raw.l3.ipv4.src_ip;
	...
	if (rc) {
		N3K_MGMT_LOG(ERR, "get_target_action_queue_num failed %d", rc);
		return rc;
	}
	...
	struct n3k_mgmt_flow_entry *flow)
{
	...
	if (ret) {
		N3K_MGMT_LOG(ERR, "calculating hash for queue id failed %d", ret);
		return ret;
	}
