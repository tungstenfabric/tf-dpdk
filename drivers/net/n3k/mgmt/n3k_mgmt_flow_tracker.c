...
typedef enum {
	N3K_MGMT_INIT_FT_TYPE_NON_CT,
	N3K_MGMT_INIT_FT_TYPE_CT,
	N3K_MGMT_INIT_FT_DIR_RSP,
} n3k_mgmt_init_ft_conn_type;
	...
	struct flow_tracker_data_regs *ft_regs, uint8_t ip_prot)
{
	...
	ft_regs->data_7.field.conn_type = N3K_MGMT_INIT_FT_TYPE_NON_CT;
	ft_regs->data_7.field.state = N3K_MGMT_INIT_TCP_CONNTRACK_NONE;
	if (ip_prot == IPPROTO_TCP) {
		// TODO(n3k): Add support for different stages of TCP connection
		ft_regs->data_7.field.conn_type = N3K_MGMT_INIT_FT_TYPE_CT;
		ft_regs->data_7.field.state = N3K_MGMT_INIT_TCP_CONNTRACK_ESTABLISHED;
	} else if (ip_prot == IPPROTO_UDP) {
		ft_regs->data_7.field.conn_type = N3K_MGMT_INIT_FT_TYPE_CT;
		ft_regs->data_7.field.state = N3K_MGMT_INIT_UDP_CONNTRACK_INIT;
	}
}
	...
	const struct flow_tracker_data_regs *ft_regs)
{
	...
	if (ft_stat.field.fatal_error) {
		n3k_mgmt_unlock(hw);
		N3K_MGMT_LOG(DEFAULT, ERR, "flow tracker fatal error");
		return -EFAULT;
	}
	...
	struct n3k_mgmt_flow_tracker_stats *ft_stats)
{
	...
	n3k_mgmt_flow_tbl_set_cached_stats(handle, ft_stats);
