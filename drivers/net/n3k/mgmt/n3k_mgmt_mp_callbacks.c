	...
	struct n3k_mgmt_mp_table_iterator *iterator)
{
	...
	case N3K_MGMT_MP_REQUEST_TABLE_FLOW_RAW:
		return n3k_mgmt_flow_tbl_first_no_mirror_handle(hw, &iterator->flow_tbl_iterator);
	...
	N3K_MGMT_LOG(MP, ERR, "Invalid request type %d", type);
	return -EINVAL;
}
	...
	struct n3k_mgmt_mp_table_iterator *iterator)
{
	...
	case N3K_MGMT_MP_REQUEST_TABLE_FLOW:
	case N3K_MGMT_MP_REQUEST_TABLE_FLOW_RAW:
		return n3k_mgmt_flow_tbl_next_no_mirror_handle(hw, &iterator->flow_tbl_iterator);
	case N3K_MGMT_MP_REQUEST_TABLE_TUNNEL:
	case N3K_MGMT_MP_REQUEST_TABLE_TUNNEL_RAW:
	...
	N3K_MGMT_LOG(MP, ERR, "Invalid request type %d", iterator->type);
	return -EINVAL;
}
	...
	struct n3k_mgmt_mp_flow_response *flow)
{
	...
	ret = n3k_mgmt_flr_flow_tracker_get_sync(hw, flow_handle,
		&flow->ft);
	if (ret < 0) {
		N3K_MGMT_LOG(MP, ERR, "error while getting ft stats %d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		N3K_MGMT_LOG(MP, ERR, "error while getting flow tbl entry %d", ret);
		return ret;
	}

	ret = n3k_mgmt_get_key_action_tbl_entries(hw, &tbl_entry, &flow->flow);
	if (ret < 0) {
		N3K_MGMT_LOG(MP, ERR, "error while getting key and action tbl entries %d", ret);
		return ret;
	}
	...
	struct n3k_mgmt_mp_total_entries_response *response)
{
	...
	if (ret) {
		N3K_MGMT_LOG(MP, ERR, "sync read reg failed %d", ret);
		return ret;
	}
	...
	uint32_t *value)
{
	...
	switch (hw->insert_type) {
		...
		case N3K_MGMT_HW_INSERT_TYPE_FLR_SIM:
			...
			if (ret) {
				N3K_MGMT_LOG(MP, ERR, "sync read reg failed %d",
					ret);
			}
			return ret;
		case N3K_MGMT_HW_INSERT_TYPE_CSR:
...
void
n3k_mgmt_mp_fill_vf0_stats_response(struct n3k_mgmt_hw *hw,
	struct n3k_mgmt_mp_vf0_stats_response *stats)
{
	stats->flr.rx_pkts = rte_atomic64_read(&hw->vf0_stats.flr.rx_pkts);
	stats->flr.tx_pkts = rte_atomic64_read(&hw->vf0_stats.flr.tx_pkts);
	stats->slow_path.rx_pkts = rte_atomic64_read(&hw->vf0_stats.slow_path.rx_pkts);
	stats->slow_path.tx_pkts = rte_atomic64_read(&hw->vf0_stats.slow_path.tx_pkts);
}
