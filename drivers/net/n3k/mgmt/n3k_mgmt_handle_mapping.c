...
n3k_mgmt_handle_mapping_init(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_LOG(FLOW, INFO, "Initializing handle mapping");

	if (hw->handle_map_internal) {
		N3K_MGMT_LOG(FLOW, ERR, "Handle mapping already initialized");
		return -EINVAL;
	}
	...
	if (!hw->handle_map_internal) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to allocate handle_map_internal");
		return -ENOMEM;
	}
	...
	if (!hw->handle_map_internal->local_to_hw) {
		N3K_MGMT_LOG(FLOW, ERR, "Failed to allocate local_to_hw array");
		ret = -ENOMEM;
		goto free_handles;
	}
	...
	uint32_t local_handle, uint32_t hw_handle)
{
	...
	N3K_MGMT_LOG(FLOW, DEBUG, "Mapping local:%" PRIu32 "->hw:%" PRIu32,
	...
	uint32_t local_handle)
{
	...
	N3K_MGMT_LOG(FLOW, DEBUG, "Unmapping local:%" PRIu32 "->hw:%" PRIu32,
