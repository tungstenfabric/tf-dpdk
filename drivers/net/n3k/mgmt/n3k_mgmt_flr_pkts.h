...
/* Values used for inband packet creation */
...
/* We enumerate number of blocks from 0 in inband packets so max number of blocks
 * is actually greater by 1
 */
#define N3K_MGMT_FLR_MAX_BLOCKS_NUM (N3K_MGMT_FLR_CONFIG_HEADER_MAX_BLOCKS_NUM + 1)
...
struct n3k_mgmt_flr_parsed_pkt {
	...
	struct n3k_mgmt_flr_parsed_block blocks[N3K_MGMT_FLR_MAX_BLOCKS_NUM];
};

struct n3k_mgmt_flr_parsed_block_range {
	uint8_t blocks_num;
	struct n3k_mgmt_flr_parsed_block blocks[N3K_MGMT_FLR_MAX_BLOCKS_NUM];
};

bool n3k_mgmt_flr_contains_error_block(
	const struct n3k_mgmt_flr_parsed_block *blocks, size_t blocks_num);
