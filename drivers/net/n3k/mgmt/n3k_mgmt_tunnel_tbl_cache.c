...
int n3k_mgmt_tunnel_tbl_cache_next_index(struct n3k_mgmt_hw *hw, uint16_t *tunnel_id)
{
	...
	N3K_MGMT_LOG(TUNNEL, DEBUG, "Started iterating: tunnel_id=%" PRIu16, *tunnel_id + 1);
	...
	for (*tunnel_id = *tunnel_id + 1;
		...
		if (rte_atomic32_read(&cur_id_to_key->ref_count)) {
			N3K_MGMT_LOG(TUNNEL, DEBUG, "Found tunnel entry: tunnel_id=%" PRIu16,
				*tunnel_id);
			return 0;
		}
	}

	N3K_MGMT_LOG(TUNNEL, DEBUG, "Entry not found");
...
int n3k_mgmt_tunnel_tbl_cache_init(struct n3k_mgmt_hw *hw)
{
	...
	N3K_MGMT_LOG(TUNNEL, INFO, "Initializing tunnel tbl cache");
	...
	uint16_t* tunnel_id)
{
	...
	if (N3K_MGMT_SHOULD_LOG(TUNNEL, DEBUG)) {
		N3K_MGMT_LOG(TUNNEL, DEBUG, "Fetching tunnel tbl cache entry:");
	...
	N3K_MGMT_LOG(TUNNEL, DEBUG, "Key not found in cache. Adding new tunnel.");
	...
	if (i == N3K_MGMT_TUNNEL_TBL_MAX_ENTRIES) {
		// all entries used
		N3K_MGMT_LOG(TUNNEL, WARNING, "No free entry in cache");
		rc = -ENOSPC;
		goto release_lock;
	}
	...
	*tunnel_id = i;
	...
	rc = rte_hash_add_key_data(hw->tunnel_cache->key_to_id, key_rfcnt->key,
		(void*)id_as_ptr);
	if (rc < 0) {
		N3K_MGMT_LOG(TUNNEL, ERR, "Error while adding entry to cache");
	...
	uint16_t* tunnel_id)
{
	...
	if (rc < 0) {
		if (N3K_MGMT_SHOULD_LOG(TUNNEL, DEBUG)) {
			N3K_MGMT_LOG(TUNNEL, DEBUG, "Couldn't find id for key: ret=%d", rc);
			n3k_mgmt_tunnel_tbl_entry_dump(key);
		}
		return rc;
	}
	...
	uint16_t tunnel_id, struct n3k_mgmt_tunnel_tbl_entry* out_key)
{
	...
	if (tunnel_id >= N3K_MGMT_TUNNEL_TBL_MAX_ENTRIES) {
		N3K_MGMT_LOG(TUNNEL, DEBUG, "Invalid tunnel_id=%" PRIu16, tunnel_id);
		return -ENOENT;
	}
	...
	if (N3K_MGMT_SHOULD_LOG(TUNNEL, DEBUG)) {
		N3K_MGMT_LOG(TUNNEL, DEBUG, "Found entry by id: tunnel_id=%" PRIu16, tunnel_id);
...
int n3k_mgmt_tunnel_tbl_cache_yield(struct n3k_mgmt_hw *hw, uint16_t tunnel_id)
{
	...
	if (tunnel_id >= N3K_MGMT_TUNNEL_TBL_MAX_ENTRIES) {
		N3K_MGMT_LOG(TUNNEL, ERR, "Invalid tunnel_id=%" PRIu16, tunnel_id);
		return -ENOENT;
	}
