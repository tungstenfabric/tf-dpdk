...
n3k_mgmt_flr_sim_init(struct n3k_mgmt_hw *hw, const struct n3k_mgmt_args *args)
{
	...
	N3K_MGMT_LOG(FLR_SIM, INFO, "flr simulator queues initalization succeeded");

	ret = n3k_mgmt_flr_sim_request_handler_init(hw);
	if (ret) {
		N3K_MGMT_LOG(FLR_SIM, ERR,
			"n3k_mgmt_flr_sim_request_handler_init failed. ret = %d",
			ret);
		return ret;
	}
	N3K_MGMT_LOG(FLR_SIM, INFO, "flr simulator request handler initalization succeeded");
