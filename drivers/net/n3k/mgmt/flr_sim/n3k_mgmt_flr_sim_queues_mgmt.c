...
n3k_mgmt_flr_sim_init_queues(struct n3k_mgmt_hw *hw)
{
	...
	if (flr_sim_queues_mgmt_internal->req_queue == NULL) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "failed to allocate rte_ring(%s)", N3K_MGMT_FLR_REQ_RING_NAME);
		return -rte_errno;
	}
	...
	if (flr_sim_queues_mgmt_internal->resp_queue == NULL) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "failed to allocate rte_ring(%s)", N3K_MGMT_FLR_RESP_RING_NAME);
...
{
	const uint64_t rand_max = UINT64_MAX;
	...
	const struct n3k_mgmt_flr_sim_queues_sim_params *params)
{
	...
	if (hw->flr_sim_queues_mgmt_internal) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Flr simulator queues already initialized");
		return -EINVAL;
	}
	...
	if (!hw->flr_sim_queues_mgmt_internal) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Failed to allocate flr_sim_queues_mgmt_internal");
		return -ENOMEM;
	}

	ret = init_action_probs(hw, params);
	if (ret < 0) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Failed to init flr sim probabilities");
		goto free_internal;
	}

	ret = n3k_mgmt_flr_sim_init_queues(hw);
	if (ret) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Failed to setup flr sim queues");
		goto free_internal;
	}

	ret = n3k_mgmt_flr_sim_setup_service(hw);
	if (ret) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Failed to setup flr sim queues mgmt service");
		goto free_queues;
	}

	ret = n3k_mgmt_flr_sim_init_mempool(hw);
	if (ret) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Failed to setup flr sim mbuf mempool");
...
copy_mbuf_to_packet_data(const struct rte_mbuf *pkt, struct packet_data **pkt_data)
{
	...
	*pkt_data = n3k_malloc("flr_sim_packet_data", sizeof(**pkt_data), 0);
	if (*pkt_data == NULL) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "failed to allocate memory for packet data");
		return -EINVAL;
	}
...
copy_packet_data_to_mbuf(const struct packet_data *pkt_data, struct rte_mbuf *pkt)
{
	...
	if (added_data_ptr == NULL) {
		N3K_MGMT_FLR_SIM_LOG(ERR, "Error in resizing packet");
		return -ENOMEM;
	}
...
n3k_mgmt_flr_handle_requests_service_callback(void *args)
{
	...
	while (!rte_ring_dequeue(flr_sim_queues_mgmt_internal->req_queue, (void**)&request_response)) {
		N3K_MGMT_FLR_SIM_LOG(DEBUG, "Dequeuing request pkt from queue");

		if (request_response->action == ACTION_SLEEP) {
			uint32_t sleep_ms = rte_rand() % 1500;
			N3K_MGMT_FLR_SIM_LOG(DEBUG, "Sleeping before executing request");
		...
		if (ret < 0) {
			...
			N3K_MGMT_FLR_SIM_LOG(ERR, "failed to execute request; ret = %d", ret);
			continue;
		}

		N3K_MGMT_FLR_SIM_LOG(DEBUG, "Enqueuing response pkt to queue");

		if (request_response->action != ACTION_DROP_RX) {
			ret = rte_ring_enqueue(flr_sim_queues_mgmt_internal->resp_queue, request_response);
			if (ret < 0) {
				free_packet_data(request_response);
				N3K_MGMT_FLR_SIM_LOG(ERR, "failed to push flr response; ret = %d", ret);
				continue;
			}
		} else {
			N3K_MGMT_FLR_SIM_LOG(DEBUG, "Dropping response packet");
			free_packet_data(request_response);
		}
	}
	return 0;
...
n3k_mgmt_flr_sim_tx(struct n3k_mgmt_hw *hw, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; ++i) {
		...
		N3K_MGMT_FLR_SIM_LOG(DEBUG, "Enqueuing request pkt to queue");

		ret = copy_mbuf_to_packet_data(tx_pkts[i], &request);
		if (ret < 0) {
			N3K_MGMT_FLR_SIM_LOG(ERR, "failed to copy request pkt; ret = %d", ret);
			break;
		}
		...
		if (request->action != ACTION_DROP_TX) {
			...
			if (ret < 0) {
				...
				N3K_MGMT_FLR_SIM_LOG(ERR, "failed to push flr request; ret = %d", ret);
				break;
			}
		} else {
			N3K_MGMT_FLR_SIM_LOG(DEBUG, "Dropping request packet");
			free_packet_data(request);
		}
...
n3k_mgmt_flr_sim_rx(struct n3k_mgmt_hw *hw, struct rte_mbuf **rx_pkts, uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		if (rx_pkts[i] == NULL) {
			N3K_MGMT_FLR_SIM_LOG(ERR, "failed to allocate mbuf");
			break;
		}
		...
		N3K_MGMT_FLR_SIM_LOG(DEBUG, "Dequeuing response pkt from queue");
