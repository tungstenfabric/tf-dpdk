...
void
ifc_hw_mq_enable(struct n3k_mgmt_hw *hw)
{
	// read modify write
...
void
ifc_hw_max_vq_pairs_set(struct n3k_mgmt_hw *hw, u8 idx, u16 vqp)
{
...
void
ifc_hw_virtio_features_set(struct n3k_mgmt_hw *hw, u8 idx, bool is_mq)
{
	u64 features = 0;
...
void
ifc_hw_vqs_set(struct n3k_mgmt_hw *hw, u8 idx, u16 vqs)
{
...
void
ifc_hw_vf_map_set(struct n3k_mgmt_hw *hw, u8 idx, u8 is_pf, u16 vqs, u32 offset)
{
...
static void
ifc_hw_msix_map_set(struct n3k_mgmt_hw *hw, u16 idx, VIRTIO_DMA_CSR_MSIX_QID_XLATE_CFG_t *msix)
{
	int tx_rx;
...
void
ifc_hw_queue_map_set(struct n3k_mgmt_hw *hw, struct ifc_vq *vqs)
{
	struct ifc_vq *it;
	u8 size = vqs->size;
	int vq_idx = 0;
	...
	for (it = vqs; it != vqs + size; ++it) {
		...
		ifc_hw_msix_map_set(hw, it->id, &msix_qid_xlate_cfg);
...
void
ifc_hw_pvf_xlate_set(struct n3k_mgmt_hw *hw, struct ifc_vq *vqs)
{
	u16 start;
	u16 entries = 0;
...
void
ifc_hw_pvf_xlate_unset(struct n3k_mgmt_hw *hw, struct ifc_vq *vqs)
{
	if (vqs->is_pf)
		csr_write(hw, MKS_CHIP_VIRTIO_DMA_CSR_BASE,
			VIRTIO_DMA_CSR_MSIX_PF_XLATE_CFG(vqs->pf),
...
void
ifc_hw_vf_unmap(struct n3k_mgmt_hw *hw, u8 idx)
{
	VIRTIO_DMA_CSR_VIC_QOFFSET_ROM_t vic_qoffset_rom = { .val = 0 };
...
void
ifc_hw_queue_unmap(struct n3k_mgmt_hw *hw, struct ifc_vq *vqs)
{
	struct ifc_vq *it;
	u8 size = vqs->size;
	int vq_idx = 0;
